...
static llvm::cl::opt<unsigned long long> cxtBudget(
    "cxtbg", llvm::cl::init(10000),
...
void ContextDDA::handleOutOfBudgetDpm(const CxtLocDPItem &dpm) {
    ...
    DBOUT(DGENERAL, outs() << "~~~Out of budget query, downgrade to flow "
                              "sensitive analysis \n");
    flowDDA->computeDDAPts(dpm.getCurNodeID());
    const PointsTo &flowPts = flowDDA->getPts(dpm.getCurNodeID());
    CxtPtSet cxtPts;
    for (PointsTo::iterator it = flowPts.begin(), eit = flowPts.end();
         it != eit; ++it) {
        ContextCond cxt;
        CxtVar var(cxt, *it);
        cxtPts.set(var);
    ...
    if (edge->isCallVFGEdge()) {
        ...
        if (CallSiteID csId = getCSIDAtCall(dpm, edge)) {
            ...
            if (isEdgeInRecursion(csId)) {
                DBOUT(DDDA, outs() << "\t\t call edge "
                                   << getPTACallGraph()
                                          ->getCallerOfCallSite(csId)
                                          ->getName()
                                   << "=>"
                                   << getPTACallGraph()
                                          ->getCalleeOfCallSite(csId)
                                          ->getName()
                                   << "in recursion \n");
        ...
        if (CallSiteID csId = getCSIDAtRet(dpm, edge)) {
            ...
            if (isEdgeInRecursion(csId)) {
                ...
                /// TODO: When this call site id is contained in current call
                /// string, we may find a recursion. Try
                ///       to solve this later.
                if (dpm.getCond().containCallStr(csId)) {
                    outOfBudgetQuery = true;
                    SVFUtil::writeWrnMsg("Call site ID is contained in call "
                                         "string. Is this a recursion?");
                    return false;
                } else {
