...
static llvm::cl::opt<unsigned long long> flowBudget(
    "flowbg", llvm::cl::init(10000),
...
void FlowDDA::handleOutOfBudgetDpm(const LocDPItem &dpm) {
    ...
    const PointsTo &anderPts =
        getAndersenAnalysis()->getPts(dpm.getCurNodeID());
    updateCachedPointsTo(dpm, anderPts);
    unionPts(dpm.getCurNodeID(), anderPts);
    addOutOfBudgetDpm(dpm);
}

bool FlowDDA::testIndCallReachability(LocDPItem &, const SVFFunction *callee,
                                      CallSiteID csId) {
