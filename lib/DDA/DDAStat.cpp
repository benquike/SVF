...
DDAStat::DDAStat(FlowDDA *pta)
    : PTAStat(pta), flowDDA(pta), contextDDA(nullptr) {
    initDefault();
}
DDAStat::DDAStat(ContextDDA *pta)
    : PTAStat(pta), flowDDA(nullptr), contextDDA(pta) {
    initDefault();
}
...
void DDAStat::performStatPerQuery(NodeID ptr) {
    ...
    NumPerQueryStatMap["IndEdgeResolved"] =
        getPTA()->getNumOfResolvedIndCallEdge() - _NumOfIndCallEdgeSolved;
...
void DDAStat::performStat() {
    ...
    timeStatMap["AvgTimePerQuery"] =
        (_TotalTimeOfQueries / TIMEINTERVAL) / _TotalNumOfQuery;
    ...
    timeStatMap["AvgDPMAtLoc"] =
        (double)_AvgNumOfDPMAtSVFGNode / _TotalNumOfQuery;
...
void DDAStat::printStat() {
    ...
    if (flowDDA) {
        ...
        contextDDA->getSVFG()->getStat()->performSCCStat(
