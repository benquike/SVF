...
// static llvm::cl::opt<bool> TDPrint("print-td", llvm::cl::init(true),
...
bool PCG::mayHappenInParallelBetweenFunctions(const Function *fun1,
                                              const Function *fun2) const {
    ...
    if (isSpawneeFun(fun1) == false && isSpawneeFun(fun2) == false)
        return false;
    // if there exit one of the function are not spawner, spawnee or follower,
    // then they won't happen in parallel
    if (isSpawnerFun(fun1) == false && isSpawneeFun(fun1) == false &&
        isFollowerFun(fun1) == false)
        return false;
    if (isSpawnerFun(fun2) == false && isSpawneeFun(fun2) == false &&
        isFollowerFun(fun2) == false)
        return false;

    return true;
}

bool PCG::mayHappenInParallel(const Instruction *i1,
 ...
 * a procedure is a spawner if it creates a thread and the created thread is
 * still existent on its return a procedure is a spawnee if it is created by
 * fork call
 */
void PCG::initFromThreadAPI(SVFModule *module) {
    for (SVFModule::const_iterator fi = module->begin(), efi = module->end();
         fi != efi; ++fi) {
...
/*!
 * 	Infer spawners and spawnees from call graph. The inference are recursively
 * done spawners: procedures may create a thread and return with the created
 * thread still running spawnees: procedures may be executed as a spawned thread
 * 	followers: procedures may be invoked by a thread after the thread returns
 * from a spawner (procedure may be called after pthread_creat is called).
 */
...
void PCG::collectSpawners() {
    ...
    for (FunSet::iterator it = spawners.begin(), eit = spawners.end();
         it != eit; ++it) {
        worklist.push(*it);
    }
    while (!worklist.empty()) {
        ...
        for (PTACallGraphNode::const_iterator it = funNode->InEdgeBegin(),
                                              eit = funNode->InEdgeEnd();
             it != eit; ++it) {
            ...
            /// add all the callsites from callers to callee (spawner) as a
            /// spawn site.
            for (PTACallGraphEdge::CallInstSet::iterator
                     dit = callEdge->directCallsBegin(),
                     deit = callEdge->directCallsEnd();
                 dit != deit; ++dit) {
                addSpawnsite(*dit);
            }
            for (PTACallGraphEdge::CallInstSet::iterator
                     dit = callEdge->indirectCallsBegin(),
...
void PCG::collectSpawnees() {
    ...
    FunWorkList worklist;
    for (FunSet::iterator it = spawnees.begin(), eit = spawnees.end();
         it != eit; ++it) {
        worklist.push(*it);
    }
    while (!worklist.empty()) {
        const Function *fun = worklist.pop();
        PTACallGraphNode *funNode = callgraph->getCallGraphNode(fun);
        for (PTACallGraphNode::const_iterator it = funNode->OutEdgeBegin(),
                                              eit = funNode->OutEdgeEnd();
             it != eit; ++it) {
 ...
 * a procedure whose callsite lies in a control flow path that starts just after
 * a spawner's callsite
 */
void PCG::identifyFollowers() {

    for (CallInstSet::iterator sit = spawnSitesBegin(), esit = spawnSitesEnd();
         sit != esit; ++sit) {
                ...
                if ((SVFUtil::isa<CallInst>(inst) ||
                    ...
                    if (callgraph->hasCallGraphEdge(inst)) {
                        for (PTACallGraph::CallGraphEdgeSet::const_iterator
                                 cgIt = callgraph->getCallEdgeBegin(inst),
                                 ecgIt = callgraph->getCallEdgeEnd(inst);
                             cgIt != ecgIt; ++cgIt) {
...
/*!
 * collect follower procedures which may be called after pthread_create is
 * invoked directly or indirectly a procedure which is called from a follower is
 * also a follower.
 */
void PCG::collectFollowers() {
    ...
    FunWorkList worklist;
    for (FunSet::iterator it = followers.begin(), eit = followers.end();
         it != eit; ++it) {
        worklist.push(*it);
    }
    while (!worklist.empty()) {
 ...
 * means execution of x in one thread may overlap execution of y in another
 * thread I(x,y) is a set of memory locations for this interference edge
 */
void PCG::interferenceAnalysis() {
    ...
    PCG::FunVec worklist;
    for (SVFModule::const_iterator F = mod->begin(), E = mod->end(); F != E;
         ++F) {
    ...
    while (!worklist.empty()) {
        ...
        bool ismhpfun = false;
        for (PCG::FunVec::iterator it = worklist.begin(), eit = worklist.end();
             it != eit; ++it) {
 ...
 */
void PCG::printResults() { printTDFuns(); }
