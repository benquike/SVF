...
bool TCT::isInLoopInstruction(const Instruction *inst) {
    ...
    while (!worklist.empty()) {
        ...
        PTACallGraphNode *cgnode =
            tcg->getCallGraphNode(inst->getParent()->getParent());
        for (PTACallGraphNode::const_iterator nit = cgnode->InEdgeBegin(),
                                              neit = cgnode->InEdgeEnd();
             nit != neit; nit++) {
            for (PTACallGraphEdge::CallInstSet::iterator
                     cit = (*nit)->directCallsBegin(),
                     ecit = (*nit)->directCallsEnd();
                 cit != ecit; ++cit) {
    ...
    for (InstSet::const_iterator it = insts.begin(), eit = insts.end();
         it != eit; ++it) {
...
void TCT::markRelProcs() {
    for (ThreadCallGraph::CallSiteSet::iterator it = tcg->forksitesBegin(),
                                                eit = tcg->forksitesEnd();
         it != eit; ++it) {
        markRelProcs((*it)->getParent()->getParent());

        for (ThreadCallGraph::ForkEdgeSet::const_iterator
                 nit = tcg->getForkEdgeBegin(*it),
                 neit = tcg->getForkEdgeEnd(*it);
             nit != neit; nit++) {
    ...
    for (ThreadCallGraph::CallSiteSet::iterator it = tcg->joinsitesBegin(),
                                                eit = tcg->joinsitesEnd();
         it != eit; ++it) {
        markRelProcs((*it)->getParent()->getParent());
    }

    if (candidateFuncSet.empty())
        writeWrnMsg("We didn't recognize any fork site, this is single thread "
                    "program?");
}
...
void TCT::collectEntryFunInCallGraph() {
    for (SVFModule::const_iterator it = tcg->getModule().begin(),
                                   eit = tcg->getModule().end();
         it != eit; ++it) {
...
void TCT::collectMultiForkedThreads() {
    ...
    while (!worklist.empty()) {
        ...
        for (TCT::ThreadCreateEdgeSet::const_iterator
                 it = node->getOutEdges().begin(),
                 eit = node->getOutEdges().end();
             it != eit; ++it) {
 ...
 */
void TCT::handleCallRelation(CxtThreadProc &ctp, const PTACallGraphEdge *cgEdge,
                             CallSite cs) {
    ...
    if (cgEdge->getEdgeKind() == PTACallGraphEdge::CallRetEdge) {
        ...
        if (pushToCTPWorkList(newctp)) {
            ...
            DBOUT(DMTA, outs() << "TCT Process CallRet new ctp --";
                  newctp.dump());
        }
    }

    else if (cgEdge->getEdgeKind() == PTACallGraphEdge::TDForkEdge) {
        const CallInst *fork = llvm::cast<CallInst>(cs.getInstruction());
        ...
        if (pushToCTPWorkList(newctp)) {
            ...
            if (addTCTEdge(this->getGNode(ctp.getTid()), spawneeNode)) {
                DBOUT(DMTA, outs() << "Add TCT Edge from thread "
                                   << ctp.getTid() << "  ";
...
bool TCT::isJoinMustExecutedInLoop(const Loop *lp, const Instruction *join) {
    ...
    assert(loopheadbb->getParent() == joinbb->getParent() &&
           "should inside same function");

    const PostDominatorTree *pdt = getPostDT(loopheadbb->getParent());
    for (succ_const_iterator it = succ_begin(loopheadbb),
                             ie = succ_end(loopheadbb);
         it != ie; ++it) {
        if (lp->contains(*it)) {
...
bool TCT::isLoopHeaderOfJoinLoop(const BasicBlock *bb) {
    for (InstToLoopMap::const_iterator it = joinSiteToLoopMap.begin(),
                                       eit = joinSiteToLoopMap.end();
         it != eit; ++it) {
...
void TCT::build() {
    ...
    for (FunSet::iterator it = entryFuncSet.begin(), eit = entryFuncSet.end();
         it != eit; ++it) {
...
void TCT::getNextInsts(const Instruction *curInst, InstVec &instList) {
    ...
    if (!curInst->isTerminator()) {
        ...
        for (succ_const_iterator it = succ_begin(BB), ie = succ_end(BB);
             it != ie; ++it) {
            /// if we are sitting at the loop header, then go inside the loop
            /// but ignore loop exit
 ...
 */
void TCT::pushCxt(CallStrCxt &cxt, const Instruction *call,
    ...
    if (inSameCallGraphSCC(tcg->getCallGraphNode(caller),
                           tcg->getCallGraphNode(callee)) == false) {
        pushCxt(cxt, csId);
 ...
 */
bool TCT::matchCxt(CallStrCxt &cxt, const Instruction *call,
                   const Function *callee) {
...
void TCT::dumpCxt(CallStrCxt &cxt) {
    ...
    rawstr << "[:";
    for (CallStrCxt::const_iterator it = cxt.begin(), eit = cxt.end();
         it != eit; ++it) {
        rawstr << " ' " << *it << " ' ";
...
void TCT::print() const {
    for (TCT::const_iterator it = this->begin(), eit = this->end(); it != eit;
         ++it) {
        outs() << "TID " << it->first << "\t";
 ...
 */
TCTEdge *TCT::hasGraphEdge(TCTNode *src, TCTNode *dst,
...
TCTEdge *TCT::getGraphEdge(TCTNode *src, TCTNode *dst, TCTEdge::CEDGEK kind) {
    for (TCTEdge::ThreadCreateEdgeSet::iterator iter = src->OutEdgeBegin();
...
struct DOTGraphTraits<TCT *> : public DefaultDOTGraphTraits {
    ...
    static std::string getGraphName(TCT *graph) { return "Thread Create Tree"; }
    ...
    static std::string getEdgeAttributes(TCTNode *node, EdgeIter EI,
                                         TCT *csThreadTree) {

        TCTEdge *edge =
            csThreadTree->getGraphEdge(node, *EI, TCTEdge::ThreadCreateEdge);
        assert(edge && "No edge found!!");
        /// black edge for direct call while two functions contain indirect
        /// calls will be label with red color
        return "color=black";
    }
};
} // End namespace llvm
