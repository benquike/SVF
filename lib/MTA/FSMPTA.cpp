...
void MTASVFGBuilder::collectLoadStoreSVFGNodes() {
    ...
    for (SVFG::const_iterator it = svfg->begin(), eit = svfg->end(); it != eit;
         ++it) {
        const SVFGNode *snode = it->second;
        if (llvm::isa<LoadSVFGNode>(snode)) {
            const StmtSVFGNode *node = llvm::cast<StmtSVFGNode>(snode);
            if (node->getInst()) {
                ldnodeSet.insert(node);
            }
        }
        if (llvm::isa<StoreSVFGNode>(snode)) {
            const StmtSVFGNode *node = llvm::cast<StmtSVFGNode>(snode);
            if (node->getInst()) {
...
SVFGEdge *MTASVFGBuilder::addTDEdges(NodeID srcId, NodeID dstId,
                                     PointsTo &pts) {

    SVFGNode *srcNode = svfg->getSVFGNode(srcId);
    SVFGNode *dstNode = svfg->getSVFGNode(dstId);

    if (SVFGEdge *edge = svfg->hasThreadVFGEdge(srcNode, dstNode,
                                                SVFGEdge::TheadMHPIndirectVF)) {
        assert(llvm::isa<IndirectSVFGEdge>(edge) &&
               "this should be a indirect value flow edge!");
        return (llvm::cast<IndirectSVFGEdge>(edge)->addPointsTo(pts) ? edge
                                                                     : nullptr);
    } else {
...
void MTASVFGBuilder::performRemovingMHPEdges() {
    while (!recordedges.empty()) {
        ...
        const auto *n1 =
            llvm::cast<StmtSVFGNode>(svfg->getSVFGNode(edgepair.first));
        const auto *n2 =
            llvm::cast<StmtSVFGNode>(svfg->getSVFGNode(edgepair.second));
        ...
        for (SVFGEdge::SVFGEdgeSetTy::iterator iter = n1->OutEdgeBegin();
             iter != n1->OutEdgeEnd(); ++iter) {
            SVFGEdge *edge = *iter;
            if (edge->isIndirectVFGEdge() && (edge->getDstNode() == n2)) {
                IndirectSVFGEdge *e = llvm::cast<IndirectSVFGEdge>(edge);
                const PointsTo &pts = e->getPointsTo();
                for (NodeBS::iterator o = remove_pts.begin(),
                                      eo = remove_pts.end();
                     o != eo; ++o) {
        ...
        while (!removededges.empty()) {
            ...
            DBOUT(DMTA, outs() << "Read Precision remove: " << edge->getSrcID()
                               << " -> " << edge->getDstID() << "\n");
...
bool MTASVFGBuilder::isHeadofSpan(const StmtSVFGNode *n,
    ...
    for (SVFGNodeIDSet::iterator it = prev.begin(), eit = prev.end(); it != eit;
         ++it) {
        assert(llvm::isa<StoreSVFGNode>(svfg->getSVFGNode(*it)) &&
               "prev is not a store node");
        const StmtSVFGNode *prevNode =
            llvm::dyn_cast<StmtSVFGNode>(svfg->getSVFGNode(*it));
...
bool MTASVFGBuilder::isTailofSpan(const StmtSVFGNode *n, InstSet mergespan) {
    ...
    for (SVFGNodeIDSet::iterator it = succ.begin(), eit = succ.end(); it != eit;
         ++it) {
        assert((llvm::isa<StoreSVFGNode>(svfg->getSVFGNode(*it)) ||
                llvm::isa<LoadSVFGNode>(svfg->getSVFGNode(*it))) &&
...
bool MTASVFGBuilder::isTailofSpan(const StmtSVFGNode *n,
                                  LockAnalysis::LockSpan lspan) {
    assert(llvm::isa<StoreSVFGNode>(n) && "Node is not a store node");
    ...
    for (SVFGNodeIDSet::iterator it = succ.begin(), eit = succ.end(); it != eit;
        ...
        assert((llvm::isa<StoreSVFGNode>(svfg->getSVFGNode(*it)) ||
                llvm::isa<LoadSVFGNode>(svfg->getSVFGNode(*it))) &&
               "succ is not a store/load node");
        if (llvm::isa<LoadSVFGNode>(svfg->getSVFGNode(*it)))
            continue;
        const StmtSVFGNode *succNode =
            llvm::dyn_cast<StmtSVFGNode>(svfg->getSVFGNode(*it));
        const Instruction *succIns = succNode->getInst();
...
MTASVFGBuilder::SVFGNodeIDSet
MTASVFGBuilder::getPrevNodes(const StmtSVFGNode *n) {
    ...
    for (SVFGEdge::SVFGEdgeSetTy::iterator iter = n->InEdgeBegin();
         iter != n->InEdgeEnd(); ++iter) {
        SVFGEdge *edge = *iter;
    ...
    while (!worklist.empty()) {
        ...
        visited.insert(node);
        if (llvm::isa<StoreSVFGNode>(node))
            prev.set(node->getId());
        else {
...
MTASVFGBuilder::SVFGNodeIDSet
MTASVFGBuilder::getSuccNodes(const StmtSVFGNode *n) {
    ...
    for (SVFGEdge::SVFGEdgeSetTy::iterator iter = n->OutEdgeBegin();
         iter != n->OutEdgeEnd(); ++iter) {
        SVFGEdge *edge = *iter;
        if (edge->isIndirectVFGEdge()) {
    ...
    while (!worklist.empty()) {
        ...
        worklist.erase(worklist.begin());
        visited.insert(node);
        if (llvm::isa<StoreSVFGNode>(node) || llvm::isa<LoadSVFGNode>(node))
            succ.set(node->getId());
        else {
...
MTASVFGBuilder::SVFGNodeIDSet
MTASVFGBuilder::getSuccNodes(const StmtSVFGNode *n, NodeID o) {
    ...
    for (SVFGEdge::SVFGEdgeSetTy::iterator iter = n->OutEdgeBegin();
        ...
        SVFGEdge *edge = *iter;
        if (edge->isIndirectVFGEdge()) {
            auto *e = llvm::cast<IndirectSVFGEdge>(edge);
...
void MTASVFGBuilder::handleStoreLoadNonSparse(const StmtSVFGNode *n1,
...
void MTASVFGBuilder::handleStoreStoreNonSparse(const StmtSVFGNode *n1,
                                               const StmtSVFGNode *n2,
                                               PointerAnalysis *pta) {
    PointsTo pts = pta->getPts(n1->getPAGDstNodeID());
...
void MTASVFGBuilder::handleStoreLoadWithLockPrecisely(const StmtSVFGNode *n1,
                                                      const StmtSVFGNode *n2,
                                                      PointerAnalysis *pta) {
    ...
    //        bool n1istail = false;
    //        for (NodeBS::iterator it1 = comlocks1.begin(), ie1 =
    //        comlocks1.end(); it1 != ie1; ++it1) {
    //            LockAnalysis::LockSpan lspan1 =
    //            lockana->getSpanfromCxtLock(*it1);
    ...
    //
    //        bool n2ishead = false;
    //        for (NodeBS::iterator it2 = comlocks2.begin(), ie2 =
    //        comlocks2.end(); it2 != ie2; ++it2) {
    //            LockAnalysis::LockSpan lspan2 =
    //            lockana->getSpanfromCxtLock(*it2);
...
void MTASVFGBuilder::handleStoreStoreWithLockPrecisely(const StmtSVFGNode *n1,
                                                       const StmtSVFGNode *n2,
                                                       PointerAnalysis *pta) {
...
void MTASVFGBuilder::mergeSpan(NodeBS comlocks, InstSet &res) {
    //    for (NodeBS::iterator it = comlocks.begin(), ie = comlocks.end(); it
    //    != ie; ++it) {
    //        LockAnalysis::LockSpan lspan = lockana->getSpanfromCxtLock(*it);
    //        for (LockAnalysis::LockSpan::const_iterator cts = lspan.begin(),
    //        ects = lspan.end(); cts!=ects; ++cts) {
    //            res.insert((*cts).getStmt());
    //        }
    //    }
}

/// For o,  n2-o->n1, n1 and n2 are write. Foreach n3:n1->n3, n2->n3; then
/// remove n2->n1.
void MTASVFGBuilder::readPrecision() {
    ...
    for (SVFGNodeSet::iterator it1 = stnodeSet.begin(), eit1 = stnodeSet.end();
         it1 != eit1; ++it1) {
        const StmtSVFGNode *n1 = llvm::cast<StmtSVFGNode>(*it1);

        for (SVFGEdge::SVFGEdgeSetTy::iterator iter = n1->InEdgeBegin();
             iter != n1->InEdgeEnd(); ++iter) {
            SVFGEdge *edge = *iter;
            if (edge->isIndirectVFGEdge() &&
                llvm::isa<StoreSVFGNode>(edge->getSrcNode())) {
                const StmtSVFGNode *n2 =
                    llvm::cast<StmtSVFGNode>(edge->getSrcNode());

                IndirectSVFGEdge *e = llvm::cast<IndirectSVFGEdge>(edge);
                PointsTo pts = e->getPointsTo();
                PointsTo remove_pts;

                for (NodeBS::iterator o = pts.begin(), eo = pts.end(); o != eo;
                     ++o) {
                    SVFGNodeIDSet succ1 = getSuccNodes(n1, *o);
                    SVFGNodeIDSet succ2 = getSuccNodes(n2, *o);

                    bool remove = true;
                    for (SVFGNodeIDSet::iterator sn1 = succ1.begin(),
                                                 esn1 = succ1.end();
                         sn1 != esn1; sn1++) {
                        if (!succ2.test(*sn1)) {
                            remove = false;
...
void MTASVFGBuilder::connectMHPEdges(PointerAnalysis *pta) {
    ...
    for (SVFGNodeSet::const_iterator it1 = stnodeSet.begin(),
        ...
        for (SVFGNodeSet::const_iterator it2 = ldnodeSet.begin(),
                                         eit2 = ldnodeSet.end();
             it2 != eit2; ++it2) {
            const StmtSVFGNode *n2 = llvm::cast<StmtSVFGNode>(*it2);
            const Instruction *i2 = n2->getInst();
            if (ADDEDGE_NONSPARSE == Options::AddModelFlag) {
                if (Options::UsePCG) {
