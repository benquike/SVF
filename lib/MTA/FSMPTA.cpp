...
static llvm::cl::opt<bool>
    UsePCG("pcgTDEdge", llvm::cl::init(false),
           llvm::cl::desc("Use PCG lock for non-sparsely adding SVFG edges"));
static llvm::cl::opt<bool> IntraLock(
    "intralockTDEdge", llvm::cl::init(true),
    llvm::cl::desc("Use simple intra-procedual lock for adding SVFG edges"));
static llvm::cl::opt<bool> ReadPrecisionTDEdge(
    "rpTDEdge", llvm::cl::init(false),
    llvm::cl::desc("perform read precision to refine SVFG edges"));

static llvm::cl::opt<u32_t> AddModelFlag(
    "addTDEdge", llvm::cl::init(0),
    llvm::cl::desc(
        "Add thread SVFG edges with models: 0 Non Add Edge; 1 NonSparse; "
        "2 All Optimisation; 3 No MHP; 4 No Alias; 5 No Lock; 6 No Read "
        "Precision."));
...
SVFGEdge *MTASVFGBuilder::addTDEdges(NodeID srcId, NodeID dstId,
    ...
    if (SVFGEdge *edge = svfg->hasThreadVFGEdge(srcNode, dstNode,
        ...
        return (SVFUtil::cast<IndirectSVFGEdge>(edge)->addPointsTo(pts)
                    ? edge
                    : nullptr);
    } else {
...
void MTASVFGBuilder::performRemovingMHPEdges() {
    while (!recordedges.empty()) {
        ...
        const auto *n1 =
            SVFUtil::cast<StmtSVFGNode>(svfg->getSVFGNode(edgepair.first));
        const auto *n2 =
            SVFUtil::cast<StmtSVFGNode>(svfg->getSVFGNode(edgepair.second));
...
bool MTASVFGBuilder::isHeadofSpan(const StmtSVFGNode *n,
    ...
    for (SVFGNodeIDSet::iterator it = prev.begin(), eit = prev.end(); it != eit;
         ++it) {
        assert(SVFUtil::isa<StoreSVFGNode>(svfg->getSVFGNode(*it)) &&
               "prev is not a store node");
        const StmtSVFGNode *prevNode =
            SVFUtil::dyn_cast<StmtSVFGNode>(svfg->getSVFGNode(*it));
...
bool MTASVFGBuilder::isTailofSpan(const StmtSVFGNode *n, InstSet mergespan) {
    ...
    for (SVFGNodeIDSet::iterator it = succ.begin(), eit = succ.end(); it != eit;
         ++it) {
        assert((SVFUtil::isa<StoreSVFGNode>(svfg->getSVFGNode(*it)) ||
                SVFUtil::isa<LoadSVFGNode>(svfg->getSVFGNode(*it))) &&
...
void MTASVFGBuilder::handleStoreLoad(const StmtSVFGNode *n1,
    ...
    if (ADDEDGE_NOMHP != AddModelFlag && !mhp->mayHappenInParallel(i1, i2))
        return;
    /// Alias
    if (ADDEDGE_NOALIAS != AddModelFlag &&
    ...
    if (ADDEDGE_NOLOCK != AddModelFlag &&
...
void MTASVFGBuilder::readPrecision() {
    ...
    for (SVFGNodeSet::iterator it1 = stnodeSet.begin(), eit1 = stnodeSet.end();
        ...
        for (SVFGEdge::SVFGEdgeSetTy::iterator iter = n1->InEdgeBegin();
            ...
            SVFGEdge *edge = *iter;
            if (edge->isIndirectVFGEdge() &&
                SVFUtil::isa<StoreSVFGNode>(edge->getSrcNode())) {
                const StmtSVFGNode *n2 =
                    SVFUtil::cast<StmtSVFGNode>(edge->getSrcNode());
