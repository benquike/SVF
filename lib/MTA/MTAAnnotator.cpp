...
static llvm::cl::opt<u32_t>
    AnnoFlag("anno", llvm::cl::init(0),
             llvm::cl::desc("prune annotated instructions: 0001 Thread Local; "
                            "0002 Alias; 0004 MHP."));
...
void MTAAnnotator::pruneThreadLocal(PointerAnalysis *pta) {
    ...
    DBOUT(DGENERAL,
          outs() << pasMsg("Run annotator prune thread local pairs\n"));
    PAG *pag = pta->getPAG();
    PointsTo nonlocalobjs;
    PointsTo worklist;

    /// find fork arguments' objects
    const PAGEdge::PAGEdgeSetTy &forkedges =
        pag->getPTAEdgeSet(PAGEdge::ThreadFork);
    ...
    /// find all non-local objects that are transitively pointed by global and
    /// fork arguments.
    ...
    /// compute all store and load instructions that may operate a non-local
    /// object.
    InstSet needannost;
    InstSet needannold;
...
void MTAAnnotator::pruneAliasMHP(PointerAnalysis *pta) {
    ...
    DBOUT(DGENERAL,
          outs() << pasMsg("Run annotator prune Alias or MHP pairs\n"));
    InstSet needannost;
    InstSet needannold;
    for (auto it1 = storeset.begin(), eit1 = storeset.end(); it1 != eit1;
         ++it1) {
        for (auto it2 = it1, eit2 = storeset.end(); it2 != eit2; ++it2) {
            ...
            if (AnnoMHP) {
                if (mhp->mayHappenInParallel(*it1, *it2) &&
                    !lsa->isProtectedByCommonLock(*it1, *it2)) {
                    needannost.insert(*it1);
                    needannost.insert(*it2);
                }
            } else {
                /// if it1 == it2, mhp analysis will annotate it1 that locates
                /// in loop or recursion. but alias analysis fails to determine
                /// whether it1 is in loop or recursion, that means all store
                /// instructions will be annotated by alias analysis to
                /// guarantee sound.
                needannost.insert(*it1);
                needannost.insert(*it2);
            }
