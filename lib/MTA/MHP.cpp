...
static llvm::cl::opt<bool>
    PrintInterLev("print-interlev", llvm::cl::init(false),
                  llvm::cl::desc("Print Thread Interleaving Results"));
static llvm::cl::opt<bool> DoLockAnalysis("lockanalysis", llvm::cl::init(true),
 ...
 */
static const SCEV *getSCEVMinusExpr(const SCEV *s1, const SCEV *s2,
                                    ScalarEvolution *SE) {
 ...
 */
MHP::MHP(TCT *t)
    : tcg(t->getThreadCallGraph()), tct(t), numOfTotalQueries(0),
      numOfMHPQueries(0), interleavingTime(0), interleavingQueriesTime(0) {
...
void MHP::analyze() {
    ...
    DOTIMESTAT(interleavingTime +=
               (interleavingEnd - interleavingStart) / TIMEINTERVAL);
}
...
void MHP::analyzeInterleaving() {
    for (TCT::const_iterator it = tct->begin(), eit = tct->end(); it != eit;
         ++it) {
...
void MHP::updateNonCandidateFunInterleaving() {
    ...
    for (SVFModule::iterator F = module->begin(), E = module->end(); F != E;
         ++F) {
        ...
        if (!tct->isCandidateFun(fun) && !isExtCall(fun)) {
            ...
            for (CxtThreadStmtSet::const_iterator it1 = tsSet.begin(),
                                                  eit1 = tsSet.end();
                 it1 != eit1; ++it1) {
                const CxtThreadStmt &cts = *it1;
                const CallStrCxt &curCxt = cts.getContext();

                for (const_inst_iterator II = inst_begin(fun),
                                         EE = inst_end(fun);
                     II != EE; ++II) {
                    ...
                    threadStmtToTheadInterLeav[newCts] |=
                        threadStmtToTheadInterLeav[cts];
...
void MHP::handleNonCandidateFun(const CxtThreadStmt &cts) {
    ...
    assert(curInst == &(curfun->getEntryBlock().front()) &&
           "curInst is not the entry of non candidate function.");
    const CallStrCxt &curCxt = cts.getContext();
    PTACallGraphNode *node = tcg->getCallGraphNode(curfun);
    for (PTACallGraphNode::const_iterator nit = node->OutEdgeBegin(),
                                          neit = node->OutEdgeEnd();
         nit != neit; nit++) {
...
void MHP::handleFork(const CxtThreadStmt &cts, NodeID rootTid) {
    ...
    if (tct->getThreadCallGraph()->hasCallGraphEdge(call)) {
        for (ThreadCallGraph::ForkEdgeSet::const_iterator
                 cgIt = tcg->getForkEdgeBegin(call),
                 ecgIt = tcg->getForkEdgeEnd(call);
             cgIt != ecgIt; ++cgIt) {
...
void MHP::handleCall(const CxtThreadStmt &cts, NodeID rootTid) {
    ...
    if (tct->getThreadCallGraph()->hasCallGraphEdge(call)) {
        for (PTACallGraph::CallGraphEdgeSet::const_iterator
                 cgIt = tcg->getCallEdgeBegin(call),
                 ecgIt = tcg->getCallEdgeEnd(call);
             cgIt != ecgIt; ++cgIt) {
...
/*!
 * Handle return instruction in the current thread scope (excluding any join
 * site)
 */
void MHP::handleRet(const CxtThreadStmt &cts) {

    PTACallGraphNode *curFunNode =
        tcg->getCallGraphNode(cts.getStmt()->getParent()->getParent());
    for (PTACallGraphNode::const_iterator it = curFunNode->getInEdges().begin(),
        ...
        for (PTACallGraphEdge::CallInstSet::const_iterator
            ...
            if (matchCxt(newCxt, *cit, curFunNode->getFunction())) {
                ...
                for (InstVec::const_iterator nit = nextInsts.begin(),
                                             enit = nextInsts.end();
                     nit != enit; ++nit) {
 ...
 *
 * Exclude sibling thread that never happen in parallel based on
 * ForkJoinAnalysis
 *
...
void MHP::updateSiblingThreads(NodeID curTid) {
    NodeBS tds = tct->getAncestorThread(curTid);
    tds.set(curTid);
    for (NodeBS::iterator cit = tds.begin(), ecit = tds.end(); cit != ecit;
         ++cit) {
        NodeBS siblingTds = tct->getSiblingThread(*cit);
        for (NodeBS::iterator it = siblingTds.begin(), eit = siblingTds.end();
             it != eit; ++it) {

            if ((isHBPair(*cit, *it) && isRecurFullJoin(*cit, curTid)) ||
                isHBPair(*it, *cit))
                continue;
...
bool MHP::isRecurFullJoin(NodeID parentTid, NodeID curTid) {
    ...
    while (!worklist.empty()) {
        ...
        for (TCT::ThreadCreateEdgeSet::const_iterator
                 it = node->getInEdges().begin(),
                 eit = node->getInEdges().end();
             it != eit; ++it) {
...
bool MHP::isMustJoin(NodeID curTid, const Instruction *joinsite) {
    ...
    return !isMultiForkedThread(curTid) &&
           !tct->isJoinSiteInRecursion(joinsite);
}
 ...
 */
const Loop *MHP::isJoinInSymmetricLoop(const CallStrCxt &cxt,
...
bool MHP::isConnectedfromMain(const Function *fun) {
    ...
    while (!worklist.empty()) {
        ...
        if ("main" == node->getFunction()->getName())
            return true;
        for (PTACallGraphNode::const_iterator nit = node->InEdgeBegin(),
                                              neit = node->InEdgeEnd();
             nit != neit; nit++) {
...
bool MHP::mayHappenInParallelInst(const Instruction *i1,
    ...
    for (CxtThreadStmtSet::const_iterator it1 = tsSet1.begin(),
                                          eit1 = tsSet1.end();
         it1 != eit1; ++it1) {
        const CxtThreadStmt &ts1 = *it1;
        NodeBS l1 = getInterleavingThreads(ts1);
        for (CxtThreadStmtSet::const_iterator it2 = tsSet2.begin(),
                                              eit2 = tsSet2.end();
             it2 != eit2; ++it2) {
...
bool MHP::mayHappenInParallelCache(const Instruction *i1,
                                   const Instruction *i2) {
    if (!tct->isCandidateFun(i1->getParent()->getParent()) &&
        !tct->isCandidateFun(i2->getParent()->getParent())) {
        FuncPair funpair = std::make_pair(i1->getParent()->getParent(),
                                          i2->getParent()->getParent());
        FuncPairToBool::const_iterator it =
            nonCandidateFuncMHPRelMap.find(funpair);
...
bool MHP::mayHappenInParallel(const Instruction *i1, const Instruction *i2) {
    ...
    DOTIMESTAT(interleavingQueriesTime +=
               (queryEnd - queryStart) / TIMEINTERVAL);

    return mhp;
}

bool MHP::executedByTheSameThread(const Instruction *i1,
                                  const Instruction *i2) {
    if (!hasThreadStmtSet(i1) || !hasThreadStmtSet(i2))
        return true;
...
void MHP::printInterleaving() {
    for (ThreadStmtToThreadInterleav::const_iterator
             it = threadStmtToTheadInterLeav.begin(),
             eit = threadStmtToTheadInterLeav.end();
         it != eit; ++it) {
        outs() << "( t" << it->first.getTid() << " , $"
               << SVFUtil::getSourceLoc(it->first.getStmt()) << "$"
               << *(it->first.getStmt()) << " ) ==> [";
 ...
 * internal wrapper class PTASCEV to record all the necessary information for
 * determining symmetric fork/join inside loops
 */
void ForkJoinAnalysis::collectSCEVInfo() {
    using CallInstSet = Set<const Instruction *>;
    using FunToFJSites = Map<const Function *, CallInstSet>;
    FunToFJSites funToFJSites;

    for (ThreadCallGraph::CallSiteSet::iterator
             it = tct->getThreadCallGraph()->forksitesBegin(),
    ...
    for (ThreadCallGraph::CallSiteSet::iterator
             it = tct->getThreadCallGraph()->joinsitesBegin(),
    ...
    for (FunToFJSites::iterator it = funToFJSites.begin(),
                                eit = funToFJSites.end();
         it != eit; ++it) {
        ScalarEvolution *SE = MTA::getSE(it->first);
        for (CallInstSet::iterator sit = it->second.begin(),
                                   esit = it->second.end();
             sit != esit; ++sit) {
            ...
            if (tct->getThreadCallGraph()->isForksite(callInst)) {
                ...
                const SCEV *forkSiteTidPtrSCEV =
                    SE->getSCEV(const_cast<Value *>(forkSiteTidPtr));
                const SCEV *baseForkTidPtrSCEV = SE->getSCEV(
                    const_cast<Value *>(getBasePtr(forkSiteTidPtr)));
                forkSiteTidPtrSCEV = getSCEVMinusExpr(forkSiteTidPtrSCEV,
                                                      baseForkTidPtrSCEV, SE);
                ...
                const SCEV *joinSiteTidPtrSCEV =
                    SE->getSCEV(const_cast<Value *>(joinSiteTidPtr));
                const SCEV *baseJoinTidPtrSCEV = SE->getSCEV(
                    const_cast<Value *>(getBasePtr(joinSiteTidPtr)));
                joinSiteTidPtrSCEV = getSCEVMinusExpr(joinSiteTidPtrSCEV,
                                                      baseJoinTidPtrSCEV, SE);
...
void ForkJoinAnalysis::analyzeForkJoinPair() {
    for (TCT::const_iterator it = tct->begin(), eit = tct->end(); it != eit;
        ...
        if (const CallInst *forkInst = ct.getThread()) {
            CallStrCxt forkSiteCxt = tct->getCxtOfCxtThread(ct);
            const Instruction *exitInst =
                getExitInstOfParentRoutineFun(rootTid);
...
void ForkJoinAnalysis::handleFork(const CxtStmt &cts, NodeID rootTid) {
    ...
    if (getTCG()->hasThreadForkEdge(call)) {
        for (ThreadCallGraph::ForkEdgeSet::const_iterator
                 cgIt = getTCG()->getForkEdgeBegin(call),
...
void ForkJoinAnalysis::handleJoin(const CxtStmt &cts, NodeID rootTid) {
    ...
    if (getTCG()->hasCallGraphEdge(call)) {
        const Instruction *forkSite =
            tct->getTCTNode(rootTid)->getCxtThread().getThread();
        ...
        /// for the join site in a loop loop which does not join the current
        /// thread we process the loop exit
        else {
            if (const Loop *joinLoop = getJoinLoop(joinSite)) {
                SmallBBVector exitbbs;
                joinLoop->getExitBlocks(exitbbs);
...
void ForkJoinAnalysis::handleCall(const CxtStmt &cts, NodeID rootTid) {
    ...
    if (getTCG()->hasCallGraphEdge(call)) {
        for (PTACallGraph::CallGraphEdgeSet::const_iterator
                 cgIt = getTCG()->getCallEdgeBegin(call),
...
void ForkJoinAnalysis::handleRet(const CxtStmt &cts) {
    ...
    PTACallGraphNode *curFunNode =
        getTCG()->getCallGraphNode(curInst->getParent()->getParent());
    for (PTACallGraphNode::const_iterator it = curFunNode->getInEdges().begin(),
                                          eit = curFunNode->getInEdges().end();
         it != eit; ++it) {
        PTACallGraphEdge *edge = *it;
 ...
 * (2) thread t1 indirectly joins thread t3 via directly joining t2 (t2 fully
 * joins its child thread t3)
 */
NodeBS ForkJoinAnalysis::getDirAndIndJoinedTid(const CxtStmt &cs) {
    ...
    for (NodeBS::iterator it = directJoinTids.begin(),
                          eit = directJoinTids.end();
         it != eit; ++it) {
        worklist.push(*it);
    }
    ...
    while (!worklist.empty()) {
        ...
        for (TCT::ThreadCreateEdgeSet::const_iterator
                 it = tct->getChildrenBegin(node),
                 eit = tct->getChildrenEnd(node);
             it != eit; ++it) {
...
/*!
 * We assume a pair of fork and join sites are must-alias if they have same
 * PTASCEV (1) SCEV not inside loop (2) SCEV inside two symmetric loops, then
 *  pointers of fork thread and join thread should have same scev start and
 * step. and should have same loop trip count
 */
bool ForkJoinAnalysis::isSameSCEV(const Instruction *forkSite,
 ...
 */
bool ForkJoinAnalysis::sameLoopTripCount(const Instruction *forkSite,
                                         const Instruction *joinSite) {
    ...
    const SCEV *forkLoopCountScev =
        forkSE->getMaxBackedgeTakenCount(forkSiteLoop);
    const SCEV *joinLoopCountScev =
        joinSE->getMaxBackedgeTakenCount(joinSiteLoop);
