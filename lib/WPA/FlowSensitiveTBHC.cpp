...
void FlowSensitiveTBHC::analyze() { FlowSensitive::analyze(); }
...
void FlowSensitiveTBHC::initialize() {
    ...
    auto *dchg = llvm::dyn_cast<DCHGraph>(getCHGraph());
...
void FlowSensitiveTBHC::backPropagate(NodeID clone) {
    ...
    if (llvm::isa<CloneGepObjPN>(cloneObj) ||
        llvm::isa<GepObjPN>(originalObj)) {
    ...
    } else if (llvm::isa<CloneFIObjPN>(cloneObj) ||
               llvm::isa<CloneDummyObjPN>(cloneObj)) {
...
bool FlowSensitiveTBHC::propAlongIndirectEdge(const IndirectSVFGEdge *edge) {
    ...
    // Since the base Andersen's analysis does NOT perform type-based heap
    // cloning, it uses only the base objects; we want to account for clones
    // too.
    PointsTo edgePtsAndClones;

    // TODO: the conditional bool may be unnecessary.
    // Adding all clones is redundant, and introduces too many calls to
    // propVarPts... This introduces performance and precision penalties. We
    // should filter out according to src.
    bool isStore = false;
    const DIType *tildet = nullptr;
    PointsTo storePts;
    if (const auto *store = llvm::dyn_cast<StoreSVFGNode>(src)) {
    ...
    for (NodeID o : pts) {
        ...
        if (auto *gep = llvm::dyn_cast<GepObjPN>(getPAG()->getGNode(o))) {
            ...
            const NodeBS &geps = getGepObjsFromMemObj(
                gep->getMemObj(), gep->getLocationSet().getOffset());
            for (NodeID g : geps) {
        ...
        if (TypeBasedHeapCloning::isBlkObjOrConstantObj(oq)) {
            ...
            if (llvm::isa<VariantGepPE>(gep->getPAGEdge())) {
                ...
                if (DCHGraph::isAgg(baseType) &&
                    baseType->getTag() != dwarf::DW_TAG_array_type &&
                    normalGep->getLocationSet().getOffset() >=
                        dchg->getNumFields(baseType)) {
                    // If the field offset is too high for this object, it is
                    // killed. It seems that a clone was made on this GEP but
                    // this is not the base (e.g. base->f1->f2), and SVF expects
                    // to operate on the base (hence the large offset). The base
                    // will have been cloned at another GEP and back-propagated,
                    // thus it'll reach here safe and sound. We ignore
                    // arrays/pointers because those are array accesses/pointer
                    // arithmetic we assume are correct. Obviously,
                    // non-aggregates cannot have their fields taken so they are
                    // spurious.
                    filterSet.set(oq);
                } else {
                    // Operate on the field and all its clones.
                    const NodeBS fieldClones = getGepObjClones(
                        oq, normalGep->getLocationSet().getOffset());
                    for (NodeID fc : fieldClones) {
                        gepToSVFGRetrievers[getOriginalObj(fc)].set(
                            gep->getId());
                        tmpDstPts.set(fc);
...
bool FlowSensitiveTBHC::processLoad(const LoadSVFGNode *load) {
    ...
    for (NodeID s : srcPts) {
        if (filterSet.test(s))
            continue;
        if (pag->isConstantObj(s) || pag->isNonPointerObj(s))
            continue;
...
bool FlowSensitiveTBHC::processStore(const StoreSVFGNode *store) {
    ...
    if (!store->getPAGEdge()->isPTAEdge()) {
        ...
        changed =
            getDFPTDataTy()->updateAllDFOutFromIn(store->getId(), 0, false);
        return changed;
    }
...
static const DIType *getVTInitType(const CopySVFGNode *copy, DCHGraph *dchg) {
    ...
    const DIType *type = llvm::dyn_cast<DIType>(mdNode);
...
bool FlowSensitiveTBHC::updateInFromIn(const SVFGNode *srcStmt, NodeID srcVar,
                                       const SVFGNode *dstStmt, NodeID dstVar) {
    // IN sets are only based on the original object.
    return getDFPTDataTy()->updateDFInFromIn(
        srcStmt->getId(), getOriginalObj(srcVar), dstStmt->getId(),
        getOriginalObj(dstVar));
}

bool FlowSensitiveTBHC::updateInFromOut(const SVFGNode *srcStmt, NodeID srcVar,
                                        const SVFGNode *dstStmt,
                                        NodeID dstVar) {
    // OUT/IN sets only have original objects.
    return getDFPTDataTy()->updateDFInFromOut(
        srcStmt->getId(), getOriginalObj(srcVar), dstStmt->getId(),
        getOriginalObj(dstVar));
}

bool FlowSensitiveTBHC::unionPtsFromIn(const SVFGNode *stmt, NodeID srcVar,
                                       NodeID dstVar) {
    // IN sets only have original objects.
    return getDFPTDataTy()->updateTLVPts(stmt->getId(), getOriginalObj(srcVar),
                                         dstVar);
}

bool FlowSensitiveTBHC::unionPtsFromTop(const SVFGNode *stmt, NodeID srcVar,
                                        NodeID dstVar) {
    // OUT sets only have original objects.
    return getDFPTDataTy()->updateATVPts(srcVar, stmt->getId(),
                                         getOriginalObj(dstVar));
}

bool FlowSensitiveTBHC::propDFInToIn(const SVFGNode *srcStmt, NodeID srcVar,
                                     const SVFGNode *dstStmt, NodeID dstVar) {
    // IN sets are only based on the original object.
    return getDFPTDataTy()->updateAllDFInFromIn(
        srcStmt->getId(), getOriginalObj(srcVar), dstStmt->getId(),
        getOriginalObj(dstVar));
}

bool FlowSensitiveTBHC::propDFOutToIn(const SVFGNode *srcStmt, NodeID srcVar,
                                      const SVFGNode *dstStmt, NodeID dstVar) {
    // OUT/IN sets only have original objects.
    return getDFPTDataTy()->updateAllDFInFromOut(
        srcStmt->getId(), getOriginalObj(srcVar), dstStmt->getId(),
        getOriginalObj(dstVar));
}
...
void FlowSensitiveTBHC::determineWhichGepsAreLoads(void) {
    for (auto &nI : *svfg) {
        ...
        if (const StmtSVFGNode *gep = llvm::dyn_cast<GepSVFGNode>(svfgNode)) {
...
const MDNode *FlowSensitiveTBHC::getRawCTirMetadata(const SVFGNode *s) {
    if (const auto *stmt = llvm::dyn_cast<StmtSVFGNode>(s)) {
        const Value *v =
            stmt->getInst() ? stmt->getInst() : stmt->getPAGEdge()->getValue();
        if (v != nullptr) {
