...
void CSC::find(NodeStack &candidates) {
    ...
    while (!candidates.empty()) {
        ...
        if (_scc->subNodes(nId).count() > 1 &&
            !isVisited(nId)) // node is actually in a cycle
            visit(nId, 0);
    }
...
void CSC::visit(NodeID nodeId, Size_t _w) {
    ...
    for (auto eit = node->directOutEdgeBegin(); eit != node->directOutEdgeEnd();
         ++eit) {
        Size_t offset;
        if (auto *gepCGEdge = llvm::dyn_cast<NormalGepCGEdge>(*eit))
    ...
    while (!_S.empty()) {
        ...
        if (_consG->getGEdge(node, backNode, ConstraintEdge::NormalGep) !=
            nullptr) {
            auto *normalGep = llvm::dyn_cast<NormalGepCGEdge>(
                _consG->getGEdge(node, backNode, ConstraintEdge::NormalGep));
            assert(normalGep != nullptr && "normalGep is nullptr");
        ...
        } else if (_consG->getGEdge(node, backNode,
                                    ConstraintEdge::VariantGep) != nullptr ||
                   _consG->getGEdge(node, backNode, ConstraintEdge::Copy) !=
                       nullptr) {
    ...
    _S.pop(); // after checking all the edges of the top node of _S, remove the
              // node
}
