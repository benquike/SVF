...
void FlowSensitive::initialize()
{
    ...
    svfg = CTirAliasEval ? svfgBuilder.buildFullSVFG(ander) : svfgBuilder.buildPTROnlySVFG(ander);
    setGraph(svfg);
...
bool FlowSensitive::processGep(const GepSVFGNode* edge)
{
    ...
    else if (const auto* normalGep = SVFUtil::dyn_cast<NormalGepPE>(edge->getPAGEdge()))
    {
        for (NodeID o : srcPts)
        {
            if (isBlkObjOrConstantObj(o))
        ...
        else if (SVFUtil::isa<FormalINSVFGNode>(dstNode) || SVFUtil::isa<ActualOUTSVFGNode>(dstNode))
        {
            ...
            for (const auto& ptd : pts)
            {
                ...
                if (isFieldInsensitive(ptd))
                {
                    ...
                    for (const auto& allField : allFields)
                    {
                        if (propVarPtsAfterCGUpdated(allField, srcNode, dstNode))
                            changed = true;
                    }
                }
            }
