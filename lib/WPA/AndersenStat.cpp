...
void AndersenStat::collectCycleInfo(ConstraintGraph *consCG) {
    ...
    for (auto it = consCG->begin(), eit = consCG->end(); it != eit; ++it) {
        // sub nodes have been removed from the constraint graph, only rep nodes
        // are left.
...
void AndersenStat::constraintGraphStat() {
    ...
    for (auto &nodeIt : *consCG) {
        totalNodeNumber++;
        if (nodeIt.second->getInEdges().empty() &&
    ...
    double avgIn =
        (double)(addrtotalIn + copytotalIn + loadtotalIn + storetotalIn) /
        cgNodeNumber;
    ...
    PTNumStatMap["NumOfCGEdge"] = consCG->getLoadCGEdges().size() +
                                  consCG->getStoreCGEdges().size() +
...
void AndersenStat::performStat() {
    ...
    for (auto iter = pta->getPAG()->begin(), eiter = pta->getPAG()->end();
        ...
        if (pta->getPAG()->isValidTopLevelPtr(
                pta->getPAG()->getPAGNode(node))) {
            totalTopLevPointers++;
    ...
    ;
    timeStatMap[AverageTopLevPointsToSetSize] =
        (double)totalTopLevPtsSize / totalTopLevPointers;
    ;
    ...
    PTNumStatMap[NumOfIndirectCallSites] =
        consCG->getIndirectCallsites().size();
