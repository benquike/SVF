...
static llvm::RegisterPass<WPAPass>
    WHOLEPROGRAMPA("wpa", "Whole Program Pointer Analysis Pass");
...
static llvm::cl::opt<bool>
    anderSVFG("svfg", llvm::cl::init(false),
              llvm::cl::desc("Generate SVFG after Andersen's Analysis"));

static llvm::cl::opt<bool>
    printAliases("print-aliases", llvm::cl::init(false),
...
void WPAPass::PrintAliasPairs(PointerAnalysis *pta) {
    ...
    for (auto lit = pag->begin(), elit = pag->end(); lit != elit; ++lit) {
        ...
        for (auto rit = lit, erit = pag->end(); rit != erit; ++rit) {
            ...
            SVFUtil::outs()
                << (result == AliasResult::NoAlias ? "NoAlias" : "MayAlias")
...
ModRefInfo WPAPass::getModRefInfo(const CallInst *callInst) {
    assert(PASelected.isSet(PointerAnalysis::AndersenWaveDiff_WPA) &&
           anderSVFG &&
