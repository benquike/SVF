...
void VersionedFlowSensitiveStat::performStat() {
    ...
    PTNumStatMap[NumOfIndirectEdgeSolved] =
        vfspta->getNumOfResolvedIndCallEdge();
...
void VersionedFlowSensitiveStat::versionStat(void) {
    ...
    for (VersionedFlowSensitive::LocVersionMap::value_type &lov :
         vfspta->consume) {
        for (VersionedFlowSensitive::ObjToVersionMap::value_type &ov :
             lov.second) {
    ...
    for (Map<NodeID, Set<Version>>::value_type &ovs : versions) {
        ...
        if (numOVersions > _MaxVersions)
            _MaxVersions = numOVersions;
        if (numOVersions == 1)
            ++_NumSingleVersion;

        for (const Set<Version>::value_type &v : vs) {
            // If the version was just over-approximate and never accessed,
            // ignore.
            // TODO: with vPtD changed there is no interface to check if the PTS
            //       exists; an emptiness check is *not* an existence check.
            if (vfspta->vPtD->getPts(vfspta->atKey(o, v)).empty())
                continue;
            ...
            if (ovPts.count() > _MaxVersionPtsSize)
                _MaxVersionPtsSize = ovPts.count();
        }
    }

    _NumUsedVersions = _NumNonEmptyVersions + _NumEmptyVersions;

    if (_NumNonEmptyVersions != 0)
        _AvgVersionPtsSize =
            (double)totalVersionPtsSize / (double)_NumUsedVersions;
...
void VersionedFlowSensitiveStat::ptsSizeStat() {
    ...
    for (auto it = vfspta->getPAG()->begin(); it != vfspta->getPAG()->end();
         ++it) {
        if (!vfspta->getPAG()->isValidTopLevelPtr(it->second))
            continue;
    ...
    if (totalValidTopLvlPointers != 0)
        _AvgTopLvlPtsSize =
            (double)totalTopLvlPtsSize / (double)totalValidTopLvlPointers;

    _TotalPtsSize += totalTopLvlPtsSize;

    if (_NumNonEmptyVersions + totalValidTopLvlPointers != 0) {
        _AvgPtsSize = (double)_TotalPtsSize /
                      (double)(_NumNonEmptyVersions + totalValidTopLvlPointers);
    }

    _MaxPtsSize = _MaxVersionPtsSize > _MaxTopLvlPtsSize ? _MaxVersionPtsSize
                                                         : _MaxTopLvlPtsSize;
}
