...
static llvm::cl::opt<bool>
    DumpSlice("dump-slice", llvm::cl::init(false),
              llvm::cl::desc("Dump dot graph of Saber Slices"));

static llvm::cl::opt<unsigned>
    cxtLimit("cxtlimit", llvm::cl::init(3),
...
void SrcSnkDDA::analyze() {
    ...
    for (auto iter = sourcesBegin(), eiter = sourcesEnd(); iter != eiter;
         ++iter) {
        setCurSlice(*iter);

        DBOUT(DGENERAL, outs()
                            << "Analysing slice:" << (*iter)->getId() << ")\n");
        ContextCond cxt;
        DPIm item((*iter)->getId(), cxt);
        forwardTraverse(item);

        /// do not consider there is bug when reaching a global SVFGNode
        /// if we touch a global, then we assume the client uses this memory
        /// until the program exits.
        if (getCurSlice()->isReachGlobal()) {
            DBOUT(DSaber, outs()
                              << "Forward analysis reaches globals for slice:"
                              << (*iter)->getId() << ")\n");
        } else {
            DBOUT(DSaber,
                  outs() << "Forward process for slice:" << (*iter)->getId()
                         << " (size = " << getCurSlice()->getForwardSliceSize()
                         << ")\n");

            for (auto sit = getCurSlice()->sinksBegin(),
                      esit = getCurSlice()->sinksEnd();
                 sit != esit; ++sit) {
                ContextCond cxt;
            ...
            DBOUT(DSaber,
                  outs() << "Backward process for slice:" << (*iter)->getId()
...
void SrcSnkDDA::FWProcessOutgoingEdge(const DPIm &item, SVFGEdge *edge) {
    ...
    // points-to on the edge indicate whether the object of source node can be
    // propagated
    ...
    if (forwardVisited(dstNode, newItem)) {
        DBOUT(DSaber,
              outs() << " node " << dstNode->getId() << " has been visited\n");
        return;
    } else
        addForwardVisited(dstNode, newItem);

    if (pushIntoWorklist(newItem))
        DBOUT(DSaber, outs() << " --> " << edge->getDstID() << ", cxt size: "
                             << newItem.getContexts().cxtSize() << ")\n");
}

/*!
 * Propagate information backward without matching context, as forward analysis
 * already did it
 */
void SrcSnkDDA::BWProcessIncomingEdge(const DPIm &, SVFGEdge *edge) {
    DBOUT(DSaber, outs() << "backward propagate from (" << edge->getDstID()
                         << " --> " << edge->getSrcID() << ")\n");
...
void SrcSnkDDA::annotateSlice(ProgSlice *slice) {
    ...
    for (auto it = slice->sinksBegin(), eit = slice->sinksEnd(); it != eit;
         ++it)
        getSVFG()->getStat()->addToSinks(*it);
    for (auto it = slice->forwardSliceBegin(), eit = slice->forwardSliceEnd();
         it != eit; ++it)
        getSVFG()->getStat()->addToForwardSlice(*it);
    for (auto it = slice->backwardSliceBegin(), eit = slice->backwardSliceEnd();
         it != eit; ++it)
...
void SrcSnkDDA::printBDDStat() {
    ...
    outs() << "BDD max live number: "
           << PathCondAllocator::getMaxLiveCondNumber() << "\n";
}
