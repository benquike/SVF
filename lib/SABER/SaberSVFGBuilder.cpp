...
void SaberSVFGBuilder::buildSVFG() {
    ...
    DBOUT(DGENERAL,
          outs() << pasMsg("\tRemove Dereference Direct SVFG Edge\n"));
...
void SaberSVFGBuilder::collectGlobals(BVDataPTAImpl *pta) {
    ...
    for (auto it = pag->begin(), eit = pag->end(); it != eit; it++) {
        ...
        if (auto *gepobj = SVFUtil::dyn_cast<GepObjPN>(pagNode)) {
            if (SVFUtil::isa<DummyObjPN>(
                    pag->getPAGNode(gepobj->getBaseNode())))
                continue;
        }
...
NodeBS &SaberSVFGBuilder::CollectPtsChain(BVDataPTAImpl *pta, NodeID id,
                                          NodeToPTSSMap &cachedPtsMap) {
    PAG *pag = svfg->getPAG();
                ...
                for (const auto *pagNode : arglist) {
                    if (pagNode->isPointer()) {
                        ...
                        svfg->addIntraDirectVFEdge(
                            ...
                            svfg->getActualParmVFGNode(pagNode, it.first)
                                ->getId());
                    }
                }
            }
