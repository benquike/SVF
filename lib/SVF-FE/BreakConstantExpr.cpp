...
// Register the pass
static llvm::RegisterPass<BreakConstantGEPs>
    BP("break-constgeps", "Remove GEP Constant Expressions");
...
//
// Return value:
//  nullptr  - This value is not a constant expression with a constant
//  expression
//          GEP within it.
...
static Instruction *convertExpression(ConstantExpr *CE, Instruction *InsertPt) {
    ...
    case Instruction::Xor: {
        ...
        NewInst = llvm::BinaryOperator::Create(
            Op, CE->getOperand(0), CE->getOperand(1), CE->getName(), InsertPt);
        break;
    }

    case Instruction::FNeg: {
        auto Op = (Instruction::UnaryOps)(CE->getOpcode());
        NewInst = llvm::UnaryOperator::Create(Op, CE->getOperand(0),
                                              CE->getName(), InsertPt);
        break;
    }
    ...
    case Instruction::ICmp: {
        ...
        NewInst = CmpInst::Create(
            Op, static_cast<CmpInst::Predicate>(CE->getPredicate()),
    ...
    case Instruction::Select:
        NewInst =
            SelectInst::Create(CE->getOperand(0), CE->getOperand(1),
...
bool BreakConstantGEPs::runOnModule(Module &module) {
    ...
    for (auto &F : module) {
        ...
        //
        // Initialize the worklist by finding all instructions that have one or
        // more operands containing a constant GEP expression.
        //
        for (auto &BB : F) {
            for (auto &i : BB) {
                //
                // Scan through the operands of this instruction.  If it is a
                // constant expression GEP, insert an instruction GEP before the
                // instruction.
                //
                Instruction *I = &i;
        ...
        while (Worklist.size()) {
            ...
            //
            // Scan through the operands of this instruction and convert each
            // into an instruction.  Note that this works a little differently
            ...
            if (auto *PHI = llvm::dyn_cast<PHINode>(I)) {
                for (unsigned index = 0; index < PHI->getNumIncomingValues();
                     ++index) {
                    //
                    // For PHI Nodes, if an operand is a constant expression
                    // with a GEP, we want to insert the new instructions in the
                    // predecessor basic block.
                    //
                    // Note: It seems that it's possible for a phi to have the
                    // same incoming basic block listed multiple times; this
                    // seems okay as long the same value is listed for the
                    // incoming block.
                    //
                    Instruction *InsertPt =
                        PHI->getIncomingBlock(index)->getTerminator();
                    if (ConstantExpr *CE =
                            hasConstantGEP(PHI->getIncomingValue(index))) {
                ...
                for (unsigned index = 0; index < I->getNumOperands(); ++index) {
                    ...
                    //
                    if (ConstantExpr *CE =
                            hasConstantGEP(I->getOperand(index))) {
