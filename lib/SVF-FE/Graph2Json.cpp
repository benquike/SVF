    ...
    for (ICFG::iterator it = begin(), eit = end(); it != eit; ++it) {
        ...
        if (IntraBlockNode *bNode = SVFUtil::dyn_cast<IntraBlockNode>(node)) {
            ...
            for (PAG::PAGEdgeList::iterator it = edges.begin(),
                ...
                edge_obj["Source Type"] =
                    getPAGNodeKindValue(edge->getSrcNode()->getNodeKind());
                edge_obj["Destination Type"] =
                    getPAGNodeKindValue(edge->getDstNode()->getNodeKind());
                edge_obj["Edge Type"] =
                    getPAGEdgeKindValue(edge->getEdgeKind());
            ...
            if (isExtCall(entry->getFun()))
                ...
                ICFGNode_Obj["Source Location"] =
                    getSourceLoc(entry->getFun()->getLLVMFun());
            }
