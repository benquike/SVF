...
static llvm::cl::opt<std::string>
    Graphtxt("graphtxt", llvm::cl::value_desc("filename"),
             llvm::cl::desc("graph txt file to build PAG"));
static llvm::cl::opt<bool> SVFMain("svfmain", llvm::cl::init(false),
...
void LLVMModuleSet::loadModules(const std::vector<std::string> &moduleNameVec) {
    //
    // To avoid the following type bugs (t1 != t3) when parsing multiple
    // modules, We should use only one LLVMContext object for multiple modules
    // in the same thread. No such problem if only one module is processed by
    // SVF.
    // ------------------------------------------------------------------
...
void LLVMModuleSet::addSVFMain() {
    ...
    if (orgMain && getModuleNum() > 0 && !init_funcs.empty()) {
        ...
        // char **
        Type *i8ptr2 =
            PointerType::getInt8PtrTy(M.getContext())->getPointerTo();
...
void LLVMModuleSet::buildFunToFunMap() {
    ...
    for (auto it = svfModule->llvmFunBegin(), eit = svfModule->llvmFunEnd();
         it != eit; ++it) {
        Function *fun = *it;
    ...
    for (auto *fdecl : funDecls) {
        ...
        if (mit == nameToFunDefMap.end())
            continue;
        FunDeclToDefMap[svfModule->getSVFFunction(fdecl)] =
            svfModule->getSVFFunction(mit->second);
    }
    ...
    for (auto *fdef : funDefs) {
        ...
        if (mit == nameToFunDeclsMap.end())
            continue;
        std::vector<const SVFFunction *> &decls =
            FunDefToDeclsMap[svfModule->getSVFFunction(fdef)];
...
void LLVMModuleSet::buildGlobalDefToRepMap() {
    ...
    for (auto it = svfModule->global_begin(), eit = svfModule->global_end();
         it != eit; ++it) {
        GlobalVariable *global = *it;
