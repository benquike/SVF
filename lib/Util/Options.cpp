...
llvm::cl::bits<PointerAnalysis::PTATY> Options::DDASelected(
    ...
    llvm::cl::values(
        clEnumValN(PointerAnalysis::FlowS_DDA, "dfs",
...
const llvm::cl::opt<bool>
    Options::CallGraphDotGraph("dump-callgraph", llvm::cl::init(false),
                               llvm::cl::desc("Dump dot graph of Call Graph"));

const llvm::cl::opt<bool>
    Options::PAGPrint("print-pag", llvm::cl::init(false),
                      llvm::cl::desc("Print PAG to command line"));

const llvm::cl::opt<unsigned> Options::IndirectCallLimit(
    "ind-call-limit", llvm::cl::init(50000),
...
const llvm::cl::opt<bool>
    Options::EnableAliasCheck("alias-check", llvm::cl::init(true),
...
// MTAAnnotator.cpp
const llvm::cl::opt<u32_t>
    Options::AnnoFlag("anno", llvm::cl::init(0),
                      llvm::cl::desc("prune annotated instructions: 0001 "
                                     "Thread Local; 0002 Alias; 0004 MHP."));

// MTAResultValidator.cpp
const llvm::cl::opt<bool>
    Options::PrintValidRes("print-mhp-validation", llvm::cl::init(false),
                           llvm::cl::desc("Print MHP Validation Results"));

// MTAStat.cpp
const llvm::cl::opt<bool>
    Options::AllPairMHP("all-pair-mhp", llvm::cl::init(false),
...
const llvm::cl::opt<std::string> Options::WriteAnder(
    "write-ander", llvm::cl::init(""),
    llvm::cl::desc("Write Andersen's analysis results to a file"));

const llvm::cl::opt<std::string> Options::ReadAnder(
    "read-ander", llvm::cl::init(""),
    llvm::cl::desc("Read Andersen's analysis results from a file"));

const llvm::cl::opt<bool>
    Options::PtsDiff("diff", llvm::cl::init(true),
                     llvm::cl::desc("Disable diff pts propagation"));

const llvm::cl::opt<bool>
    Options::MergePWC("merge-pwc", llvm::cl::init(true),
                      llvm::cl::desc("Enable PWC in graph solving"));

// FlowSensitive.cpp
const llvm::cl::opt<bool> Options::CTirAliasEval(
    "ctir-alias-eval", llvm::cl::init(false),
    llvm::cl::desc(
        "Prints alias evaluation of ctir instructions in FS analyses"));

// FlowSensitiveTBHC.cpp
/// Whether we allow reuse for TBHC.
const llvm::cl::opt<bool> Options::TBHCStoreReuse(
    "tbhc-store-reuse", llvm::cl::init(false),
    llvm::cl::desc("Allow for object reuse in at stores in FSTBHC"));

const llvm::cl::opt<bool> Options::TBHCAllReuse(
    "tbhc-all-reuse", llvm::cl::init(false),
    llvm::cl::desc("Allow for object reuse everywhere in FSTBHC"));

// TypeAnalysis.cpp
const llvm::cl::opt<bool>
    Options::GenICFG("gen-icfg", llvm::cl::init(true),
                     llvm::cl::desc("Generate ICFG graph"));

// WPAPass.cpp
const llvm::cl::opt<bool> Options::AnderSVFG(
    "svfg", llvm::cl::init(false),
    llvm::cl::desc("Generate SVFG after Andersen's Analysis"));

const llvm::cl::opt<bool>
    Options::PrintAliases("print-aliases", llvm::cl::init(false),
                          llvm::cl::desc("Print results for all pair aliases"));

llvm::cl::bits<PointerAnalysis::PTATY> Options::PASelected(
    llvm::cl::desc("Select pointer analysis"),
    llvm::cl::values(
        clEnumValN(PointerAnalysis::Andersen_WPA, "nander",
                   "Standard inclusion-based analysis"),
        clEnumValN(PointerAnalysis::AndersenLCD_WPA, "lander",
                   "Lazy cycle detection inclusion-based analysis"),
        clEnumValN(PointerAnalysis::AndersenHCD_WPA, "hander",
                   "Hybrid cycle detection inclusion-based analysis"),
        clEnumValN(PointerAnalysis::AndersenHLCD_WPA, "hlander",
                   "Hybrid lazy cycle detection inclusion-based analysis"),
        clEnumValN(PointerAnalysis::AndersenSCD_WPA, "sander",
                   "Selective cycle detection inclusion-based analysis"),
        clEnumValN(
            PointerAnalysis::AndersenSFR_WPA, "sfrander",
            "Stride-based field representation includion-based analysis"),
        clEnumValN(PointerAnalysis::AndersenWaveDiff_WPA, "wander",
                   "Wave propagation inclusion-based analysis"),
        clEnumValN(PointerAnalysis::AndersenWaveDiff_WPA, "ander",
                   "Diff wave propagation inclusion-based analysis"),
        clEnumValN(PointerAnalysis::Steensgaard_WPA, "steens",
                   "Steensgaard's pointer analysis"),
        // Disabled till further work is done.
        // clEnumValN(PointerAnalysis::AndersenWaveDiffWithType_WPA,
        // "andertype", "Diff wave propagation with type inclusion-based
        // analysis"),
        clEnumValN(PointerAnalysis::FSSPARSE_WPA, "fspta",
                   "Sparse flow sensitive pointer analysis"),
        clEnumValN(
            PointerAnalysis::FSTBHC_WPA, "fstbhc",
            "Sparse flow-sensitive type-based heap cloning pointer analysis"),
        clEnumValN(PointerAnalysis::VFS_WPA, "vfspta",
                   "Versioned sparse flow-sensitive points-to analysis"),
        clEnumValN(PointerAnalysis::TypeCPP_WPA, "type",
                   "Type-based fast analysis for Callgraph, PAG and CHA")));

llvm::cl::bits<WPAPass::AliasCheckRule> Options::AliasRule(
    llvm::cl::desc("Select alias check rule"),
    llvm::cl::values(clEnumValN(WPAPass::Conservative, "conservative",
