 ...
 */
std::string SVFUtil::sucMsg(std::string msg) { return KGRN + msg + KNRM; }
 ...
 */
std::string SVFUtil::errMsg(std::string msg) { return KRED + msg + KNRM; }

std::string SVFUtil::bugMsg1(std::string msg) { return KYEL + msg + KNRM; }

std::string SVFUtil::bugMsg2(std::string msg) { return KPUR + msg + KNRM; }

std::string SVFUtil::bugMsg3(std::string msg) { return KCYA + msg + KNRM; }

/*!
 * print each pass/phase message by converting a string into blue string output
 */
std::string SVFUtil::pasMsg(std::string msg) { return KBLU + msg + KNRM; }
...
std::string SVFUtil::getSourceLoc(const Value *val) {
    ...
    if (const auto *inst = llvm::dyn_cast<Instruction>(val)) {
        if (llvm::isa<AllocaInst>(inst)) {
            for (llvm::DbgInfoIntrinsic *DII :
                ...
                if (auto *DDI = llvm::dyn_cast<llvm::DbgDeclareInst>(DII)) {
                    ...
                    rawstr << "ln: " << DIVar->getLine()
                           << " fl: " << DIVar->getFilename();
                    break;
                }
            }
        ...
        {
            auto *Loc =
                llvm::cast<llvm::DILocation>(N); // DILocation is in DebugInfo.h
    ...
    } else if (const BasicBlock *bb = llvm::dyn_cast<BasicBlock>(val)) {
...
const std::string SVFUtil::value2String(const Value *value) {
    ...
    raw_string_ostream rawstr(str);
    if (value) {
        if (const SVF::Function *fun = llvm::dyn_cast<Function>(value))
            rawstr << " " << fun->getName() << " ";
        else
            rawstr << " " << *value << " ";
