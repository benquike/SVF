...
static llvm::cl::opt<bool>
    PrintPathCond("print-pc", llvm::cl::init(false),
...
/*!
 * Allocate conditions for a basic block and propagate its condition to its
 * successors.
 */
void PathCondAllocator::allocateForBB(const BasicBlock &bb) {
    ...
    // if successor number greater than 1, allocate new decision variable for
    // successors
    if (succ_number > 1) {
        ...
        for (succ_const_iterator succ_it = succ_begin(&bb);
             succ_it != succ_end(&bb); succ_it++, succ_index++) {
            ...
            // decide whether each bit of succ_index is 1 or 0, if (three
            // successor) succ_index is 000 then use C1^C2^C3 if 001 use
            // C1^C2^negC3
            for (u32_t j = 0; j < bit_num; j++) {
                // test each bit of this successor's index (binary
                // representation)
                u32_t tool = 0x01 << j;
                if (tool & succ_index) {
 ...
 */
PathCondAllocator::Condition *
PathCondAllocator::getBranchCond(const BasicBlock *bb,
...
void PathCondAllocator::setBranchCond(const BasicBlock *bb,
                                      const BasicBlock *succ, Condition *cond) {
    ...
    /// FIXME: llvm getNumSuccessors allows duplicated block in the successors,
    /// it makes this assertion fail In this case we may waste a condition
    /// allocation, because the overwrite of the previous cond
    // assert(condPosMap.find(pos) == condPosMap.end() && "this branch has
    // already been set ");
 ...
 */
PathCondAllocator::Condition *PathCondAllocator::evaluateTestNullLikeExpr(
    const BranchInst *brInst, const BasicBlock *succ, const Value *val) {
 ...
 */
PathCondAllocator::Condition *
PathCondAllocator::evaluateProgExit(const BranchInst *brInst,
 ...
 * bb is loop header and succ is the only exit basic block outside the loop
 * (excluding exit bbs which call program exit) for all other case, we
 * conservatively evaluate false for now
 */
PathCondAllocator::Condition *
PathCondAllocator::evaluateLoopExitBranch(const BasicBlock *bb,
                                          const BasicBlock *dst) {
    const Function *fun = bb->getParent();
    assert(fun == dst->getParent() &&
    ...
    if (loopInfo->isLoopHeader(const_cast<BasicBlock *>(bb))) {
        ...
        /// if the dst dominate all other loop exit bbs, then dst can certainly
        /// be reached
        bool allPDT = true;
        PostDominatorTree *pdt = getPostDT(fun);
        for (auto *filteredbb : filteredbbs) {
            if (pdt->dominates(dst, filteredbb) == false)
                allPDT = false;
        }
 ...
 */
PathCondAllocator::Condition *PathCondAllocator::evaluateBranchCond(
    const BasicBlock *bb, const BasicBlock *succ, const Value *val) {
    if (getBBSuccessorNum(bb) == 1) {
        assert(bb->getTerminator()->getSuccessor(0) == succ &&
    ...
    if (const auto *brInst =
        ...
        Condition *evalTestNullLike =
            evaluateTestNullLikeExpr(brInst, succ, val);
        if (evalTestNullLike)
...
bool PathCondAllocator::isBBCallsProgExit(const BasicBlock *bb) {
    const Function *fun = bb->getParent();
    auto it = funToExitBBsMap.find(fun);
 ...
 * If B1 dominates B2, and B0 not dominate B2 then condition from B1-->B0 =
 * neg(B1-->B2)^(B1-->B0)
 */
PathCondAllocator::Condition *PathCondAllocator::getPHIComplementCond(
    const BasicBlock *BB1, const BasicBlock *BB2, const BasicBlock *BB0) {
    assert(BB1 && BB2 && "expect nullptr BB here!");

    DominatorTree *dt = getDT(BB1->getParent());
    /// avoid both BB0 and BB1 dominate BB2 (e.g., while loop), then BB2 is not
    /// necessaryly a complement BB
...
/*!
 * Compute calling inter-procedural guards between two SVFGNodes (from caller to
 * callee) src --c1--> callBB --true--> funEntryBB --c2--> dst the
 * InterCallVFGGuard is c1 ^ c2
 */
PathCondAllocator::Condition *
PathCondAllocator::ComputeInterCallVFGGuard(const BasicBlock *srcBB,
...
/*!
 * Compute return inter-procedural guards between two SVFGNodes (from callee to
 * caller) src --c1--> funExitBB --true--> retBB --c2--> dst the
 * InterRetVFGGuard is c1 ^ c2
 */
PathCondAllocator::Condition *PathCondAllocator::ComputeInterRetVFGGuard(
    const BasicBlock *srcBB, const BasicBlock *dstBB, const BasicBlock *retBB) {
 ...
 */
PathCondAllocator::Condition *
PathCondAllocator::ComputeIntraVFGGuard(const BasicBlock *srcBB,
                                        const BasicBlock *dstBB) {
    ...
    while (!worklist.empty()) {
        ...
        for (succ_const_iterator succ_it = succ_begin(bb);
            ...
            /// if succ post dominate bb, then we get brCond quicker by using
            /// postDT note that we assume loop exit always post dominate loop
            /// bodys which means loops are approximated only once.
            Condition *brCond;
