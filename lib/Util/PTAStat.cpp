...
const char *PTAStat::SCCDetectionTime =
    "SCCDetectTime"; ///< Total SCC detection time
const char *PTAStat::SCCMergeTime = "SCCMergeTime"; ///< Total SCC merge time

const char *PTAStat::ProcessLoadStoreTime =
    "LoadStoreTime"; ///< process load and store time
const char *PTAStat::ProcessCopyGepTime =
    "CopyGepTime"; ///< process copy and gep time
const char *PTAStat::UpdateCallGraphTime =
    "UpdateCGTime"; ///< process copy and gep time

const char *PTAStat::TotalNumOfPointers = "TotalPointers"; ///< PAG value nodes
const char *PTAStat::TotalNumOfObjects =
    "TotalObjects"; ///< Total PAG object node
const char *PTAStat::TotalNumOfFieldObjects =
    "TotalFieldObjects"; ///< Total PAG field object node
const char *PTAStat::MaxStructSize =
    "MaxStructSize"; ///< Max struct size (bytes)
const char *PTAStat::TotalNumOfEdges =
    "TotalPAGEdges"; ///< Total PAG edge number
...
const char *PTAStat::NumOfObjsHasVarStruct =
    "VarStructObj"; ///< PAG object node has struct (maybe nested with array)
const char *PTAStat::NumOfObjsHasVarArray =
    "VarArrayObj"; ///< PAG object node has array (maybe nested with struct)
const char *PTAStat::NumOfObjsHasConstStruct =
    "ConstStructObj"; ///< PAG object node has const struct (maybe nested with
                      ///< array)
const char *PTAStat::NumOfObjsHasConstArray =
    "ConstArrayObj"; ///< PAG object node has const array (maybe nested with
                     ///< struct)
const char *PTAStat::NumOfNonPtrObjs =
    "NonPtrObj"; ///< PAG object node which is non pointer type object (do not
                 ///< have pts)
const char *PTAStat::NumOfConstantObjs =
    "ConstantObj"; ///< PAG object node which is purely scalar
...
const char *PTAStat::NumOfProcessedAddrs =
    "AddrProcessed"; ///< PAG addr processed edge
const char *PTAStat::NumOfProcessedLoads =
    "LoadProcessed"; ///< PAG load processed edge
const char *PTAStat::NumOfProcessedStores =
    "StoreProcessed"; ///< PAG store processed edge
const char *PTAStat::NumOfProcessedCopys =
    "CopyProcessed"; ///< PAG copy processed edge
const char *PTAStat::NumOfProcessedGeps =
    "GepProcessed"; ///< PAG gep processed edge

const char *PTAStat::NumOfSfr =
    "NumOfSFRs"; ///< number of field representatives
const char *PTAStat::NumOfFieldExpand = "NumOfFieldExpand";

const char *PTAStat::NumOfPointers =
    "Pointers"; ///< PAG value node, each of them maps to a llvm value
const char *PTAStat::NumOfGepFieldPointers =
    "DYFieldPtrs"; ///< PAG gep value node (field value, dynamically created
                   ///< dummy node)

const char *PTAStat::NumOfMemObjects =
    "MemObjects"; ///< PAG object node, each of them maps to a llvm value
const char *PTAStat::NumOfGepFieldObjects =
    "DYFieldObjs"; ///< PAG gep object node (field obj, dynamically created
                   ///< dummy node)

const char *PTAStat::AveragePointsToSetSize =
    "AvgPtsSetSize"; ///< Average Points-to set size
const char *PTAStat::AverageTopLevPointsToSetSize =
    "AvgTopLvlPtsSize"; ///< Average Points-to set size of top level pointers
const char *PTAStat::MaxPointsToSetSize =
    "MaxPtsSetSize"; ///< Max Points-to set size

const char *PTAStat::NumOfIterations =
    "Iterations"; ///< Number of iterations during resolution

const char *PTAStat::NumOfIndirectCallSites =
    "IndCallSites"; ///< Number of indirect callsites
const char *PTAStat::NumOfIndirectEdgeSolved =
    "IndEdgeSolved"; ///< Number of indirect calledge resolved

const char *PTAStat::NumOfSCCDetection =
    "NumOfSCCDetect"; ///< Number of scc detection performed
const char *PTAStat::NumOfCycles =
    "TotalCycleNum"; ///< Number of scc cycles detected
const char *PTAStat::NumOfPWCCycles =
    "TotalPWCCycleNum"; ///< Number of pwc scc cycles detected
const char *PTAStat::NumOfNodesInCycles =
    "NodesInCycles"; ///< Number of nodes in cycles detected
const char *PTAStat::MaxNumOfNodesInSCC =
    "MaxNodesInSCC"; ///< max Number of nodes in one scc cycle
...
void PTAStat::performStat() {
    ...
    generalNumMap[TotalNumOfPointers] =
    ...
    generalNumMap[TotalNumOfEdges] = pag->getTotalEdgeNum();
    ...
    generalNumMap[NumOfGeps] = pag->getEdgeSet(PAGEdge::NormalGep).size() +
                               pag->getEdgeSet(PAGEdge::VariantGep).size();
...
void PTAStat::callgraphStat() {
    ...
    for (; it != eit; ++it) {
        ...
        for (; edgeIt != edgeEit; ++edgeIt) {
            ...
            totalEdge +=
                edge->getDirectCalls().size() + edge->getIndirectCalls().size();
            if (callgraphSCC->repNode(edge->getSrcID()) ==
                callgraphSCC->repNode(edge->getDstID())) {
                edgeInCycle += edge->getDirectCalls().size() +
