...
TypeBasedHeapCloning::TypeBasedHeapCloning(BVDataPTAImpl *pta)
    : pta(pta),
      nodeIdAllocator(
          pta->getSVFProject()->getSymbolTableInfo()->getNodeIDAllocator()) {}
...
NodeID TypeBasedHeapCloning::getAllocationSite(NodeID o) const {
    assert(objToAllocation.find(o) != objToAllocation.end() &&
           "TBHC: object has no allocation site?");
...
const NodeBS TypeBasedHeapCloning::getGepObjClones(NodeID base,
    ...
    if (const GepObjPN *baseGep = llvm::dyn_cast<GepObjPN>(baseNode)) {
    ...
    // For arrays, we want things to work as normal because an array *object* is
    // more like a pointer than a struct object.
    ...
    for (NodeID gep : gepObjs) {
        ...
        if (auto *gepNode = llvm::dyn_cast<GepObjPN>(node)) {
    ...
    if (geps.empty()) {
        ...
        if (isClone(base)) {
            // Don't use ppag->getGepObjNode because base and it's original
        ...
        if (baseType->getTag() == dwarf::DW_TAG_array_type ||
            ...
            // Get the canonical type because we got the type from the DIType
            // infrastructure directly.
            newGepType = dchg->getCanonicalType(newGepType);
        } else {
            // Must be a struct/class.
            // Don't use totalOffset because we're operating on the Gep object
            // which is our parent (i.e. field of some base, not the base
            // itself).
...
bool TypeBasedHeapCloning::init(NodeID loc, NodeID p, const DIType *tildet,
                                bool reuse, bool gep) {
    assert(dchg && "TBHC: DCHG not set!");
    bool changed = false;
    ...
    for (NodeID o : pPt) {
        ...
        // When an object is field-insensitive, we can't filter on any of the
        // fields' types. i.e. a pointer of the field type can safely access an
        ...
        const Set<const DIType *> &aggs =
            dchg->isAgg(tp) ? dchg->getAggs(tp) : Set<const DIType *>();

        NodeID prop;
        bool filter = false;
        if (tp == undefType) {
            // o is uninitialised.
            // GEP objects should never be uninitialised; type assigned at
            // creation.
            assert(!isGep(obj) && "TBHC: GEP object is untyped!");
            prop = cloneObject(o, tildet, false);
            ++numInit;
            if (!pta->isHeapMemObj(o) && !llvm::isa<DummyObjPN>(obj))
                ++numSGInit;
        } else if (fieldInsensitive && tp && dchg->isFieldOf(tildet, tp)) {
            // Field-insensitive object but the instruction is operating on a
            // field.
            prop = o;
            ++numTBWU;
            if (!pta->isHeapMemObj(o) && !llvm::isa<DummyObjPN>(obj))
            ...
            // SVF treats two consecutive GEPs as children to the same
            // load/store. Special case for aggregates. SVF will transform (for
            // example)
            //    `1: s = get struct element X from array a; 2: f = get field of
            //    struct Y from s;`
            // to `1: s = get struct element X from array a; 2: f = get field of
            // struct Y from a;` so we want the second instruction to be
            // operating on an object of type 'Struct S', not 'Array of S'.
            prop = cloneObject(o, tildet, false);
            ++numAgg;
            if (!pta->isHeapMemObj(o) && !llvm::isa<DummyObjPN>(obj))
                ++numSGAgg;
        } else if (isBase(tp, tildet) && tp != tildet &&
                   (reuse || dchg->isFirstField(tp, tildet) ||
                    (!reuse && pta->isHeapMemObj(o)))) {
            // Downcast.
            // One of three conditions:
            //  - !reuse && heap: because downcasts should not happen to
            //  stack/globals.
            //  - isFirstField because ^ can happen because when we take the
            //  field of a
            //    struct that is a struct, we get its first field, then it may
            //    downcast back to the struct at another GEP.
        ...
        if (prop != o) {
            // If we cloned, we want to keep o in p's PTS but filter it (ignore
            // it later).
            pNewPt.set(o);
            filterSet.set(o);
            // TODO: hack, sound but imprecise and unclean.
            // In the aggs case there is a difference between it being good for
            // arrays and structs. For now, just propagate both the clone and
            // the original object till a cleaner solution is found.
...
NodeID TypeBasedHeapCloning::cloneObject(NodeID o, const DIType *type, bool) {
    ...
    if (const GepObjPN *gepObj = llvm::dyn_cast<GepObjPN>(obj)) {
        ...
        clone =
            addCloneGepObjNode(gepObj->getMemObj(), gepObj->getLocationSet());

        // The base needs to know about the new clone.
        addGepToObj(clone, gepObj->getBaseNode(),
                    gepObj->getLocationSet().getOffset());

        addClone(o, clone);
        addClone(getOriginalObj(o), clone);
        // The only instance of original object of a Gep object being retrieved
        // is for IN sets and gepToSVFGRetriever in FSTBHC, so we don't care
        // that clone comes from o (we can get that by checking the base and
        // offset).
        ...
        if (const auto *fiObj = llvm::dyn_cast<FIObjPN>(obj)) {
            clone = addCloneFIObjNode(fiObj->getMemObj());
        } else {
            const auto *dummyObj = llvm::dyn_cast<DummyObjPN>(obj);
...
const MDNode *TypeBasedHeapCloning::getRawCTirMetadata(const Value *v) {
    ...
    if (const auto *inst = llvm::dyn_cast<Instruction>(v)) {
        mdNode = inst->getMetadata(cppUtil::ctir::derefMDName);
    } else if (const auto *go = llvm::dyn_cast<GlobalObject>(v)) {
...
const DIType *TypeBasedHeapCloning::getTypeFromCTirMetadata(const Value *v) {
    ...
    const auto *type = llvm::dyn_cast<DIType>(mdNode);
    if (type == nullptr) {
...
void TypeBasedHeapCloning::validateTBHCTests(SVFModule *) {
    ...
    for (u32_t i = 0; i < llvmModuleSet->getModuleNum(); ++i) {
        ...
        for (const CallBlockNode *cbn : callSites) {
            ...
            // We have a test call,
            // We want the store which stores to the pointer in question (i.e.
            // operand of the store is the pointer, and the store itself is the
            // dereference).
            ...
            do {
                if (const auto *ci = llvm::dyn_cast<CallInst>(currInst)) {
                    std::string calledFnName =
                        ci->getCalledFunction()->getName().str();
                    if (calledFnName == derefFnName ||
                        calledFnName == mangledDerefFnName) {
                        const auto *si = llvm::dyn_cast<StoreInst>(prevInst);
                        assert(si &&
            ...
            // Repeat for q.
            while (currInst != nullptr) {
                // while loop, not do-while, because we need to the next
                // instruction (current instruction is the first deref()).
                prevInst = currInst;
                currInst = currInst->getNextNonDebugInstruction();
            ...
            // Now filter both points-to sets according to the type of the
            // value.
            ...
            auto *bvpta = llvm::dyn_cast<BVDataPTAImpl>(pta);
            assert(bvpta &&
            ...
            if (pPtsFiltered.empty()) {
                msgStream << SVFUtil::wrnMsg("\t WARNING") << " : pts(" << p
                          << ") is empty\n";
            }

            if (qPtsFiltered.empty()) {
                msgStream << SVFUtil::wrnMsg("\t WARNING") << " : pts(" << q
                          << ") is empty\n";
            }
...
void TypeBasedHeapCloning::dumpStats() {
    ...
    for (NodeID o : objs) {
        ...
        for (NodeID c : clones) {
            ...
            SVFUtil::outs() << c << "{" << dchg->diTypeToStr(getType(c)) << "}";
            first = false;
        }
