...
void MRGenerator::generateMRs() {
    ...
    DBOUT(DGENERAL,
          outs() << pasMsg("\tCollect ModRef For const CallBlockNode*\n"));
...
void MRGenerator::collectModRefForLoadStore() {
    ...
    SVFModule *svfModule = pta->getSVFModule();
    for (const auto *fi : *svfModule) {
        ...
        if (Options::IgnoreDeadFun &&
            isDeadFunction(fun.getLLVMFun(), pta->getSVFModule()))
            continue;

        for (Function::const_iterator iter = fun.getLLVMFun()->begin(),
                                      eiter = fun.getLLVMFun()->end();
             iter != eiter; ++iter) {
...
void MRGenerator::collectModRefForCall() {
    ...
    SymbolTableInfo *symbolTableInfo = pta->getPAG()->getSymbolTableInfo();
    for (CallSite cs : symbolTableInfo->getCallSiteSet()) {
        const CallBlockNode *callBlockNode =
            pta->getPAG()->getICFG()->getCallBlockNode(cs.getInstruction());
 ...
 * 1) map cpts to its superset(rep) which exists in the map, otherwise its
 * superset is itself 2) adjust existing items in the map if their supersets are
 * cpts
 */
...
void MRGenerator::partitionMRs() {
    ...
    /// For now, we just collapse all refs/mods objects at callsites into one
    /// region Consider modularly partition memory regions to speed up analysis
    /// (only partition regions within function scope)
    for (auto &it : getFunToPointsToList()) {
        for (const auto &cit : it.second) {
            sortPointsTo(cit);
        }
    }
    /// Generate memory regions according to condition pts after computing
    /// superset
    for (auto &it : getFunToPointsToList()) {
 ...
 * Add indirect uses an memory object in the function
 */
bool MRGenerator::addRefSideEffectOfCallSite(const CallBlockNode *cs,
                                             const NodeBS &refs) {
    if (!refs.empty()) {
        NodeBS refset = refs;
 ...
 * Add indirect def an memory object in the function
 */
bool MRGenerator::addModSideEffectOfCallSite(const CallBlockNode *cs,
                                             const NodeBS &mods) {
    if (!mods.empty()) {
        NodeBS modset = mods;
...
void MRGenerator::collectCallSitePts(const CallBlockNode *cs) {
    ...
    RetBlockNode *retBlockNode =
        pag->getICFG()->getRetBlockNode(cs->getCallSite());

    WorkList worklist;
    if (pag->hasCallSiteArgsMap(callBlockNode)) {
        const PAG::PAGNodeList &args =
            pta->getPAG()->getCallSiteArgsList(callBlockNode);
...
/*!
 * Get all the objects in callee's modref escaped via global objects (the chain
 * pts of globals) Otherwise, the object in callee's modref would not escape
 * through globals
 */
...
bool MRGenerator::isNonLocalObject(NodeID id, const SVFFunction *curFun) const {
    ...
    /// or a local variable is in function recursion cycles
        ...
        if (const auto *local = llvm::dyn_cast<AllocaInst>(obj->getRefVal())) {
            LLVMModuleSet *modSet = pta->getSVFModule()->getLLVMModSet();
            ...
            return callGraphSCC->isInCycle(
                callGraph->getCallGraphNode(fun)->getId());
        }
    }
 ...
 */
bool MRGenerator::handleCallsiteModRef(NodeBS &mod, NodeBS &ref,
    ...
    /// the heap object.
    if (isHeapAllocExtCall(cs->getCallSite(), pta->getSVFModule())) {
    ...
    /// otherwise, we find the mod/ref sets from the
    /// callee function, who has definition and
    /// been processed
    else {
...
void MRGenerator::modRefAnalysis(PTACallGraphNode *callGraphNode,
    ...
    for (auto it = callGraphNode->InEdgeBegin(),
        ...
        for (auto cit = edge->getDirectCalls().begin(),
                  ecit = edge->getDirectCalls().end();
             cit != ecit; ++cit) {
            NodeBS mod;
            NodeBS ref;
            const CallBlockNode *cs = (*cit);
            bool modrefchanged = handleCallsiteModRef(
                mod, ref, cs, callGraphNode->getFunction());
            if (modrefchanged)
                worklist.push(edge->getSrcID());
        }
        /// handle indirect callsites
        for (auto cit = edge->getIndirectCalls().begin(),
                  ecit = edge->getIndirectCalls().end();
             cit != ecit; ++cit) {
            NodeBS mod;
            NodeBS ref;
            const CallBlockNode *cs = (*cit);
...
PointsTo MRGenerator::getModInfoForCall(const CallBlockNode *cs) {
    if (isExtCall(cs->getCallSite(), pta->getSVFModule()) &&
        !isHeapAllocExtCall(cs->getCallSite(), pta->getSVFModule())) {
        PAGEdgeList &pagEdgeList = getPAGEdgesFromInst(cs->getCallSite());
        PointsTo mods;
        for (const auto *edge : pagEdgeList) {
 ...
 */
ModRefInfo MRGenerator::getModRefInfo(const CallBlockNode *cs1,
                                      const CallBlockNode *cs2) {
    bool ref = false;
    bool mod = false;

    /// return NoModRef neither two callsites ref or mod any memory
    if (getModRefInfo(cs1) == ModRefInfo::NoModRef ||
        getModRefInfo(cs2) == ModRefInfo::NoModRef)
        return ModRefInfo::NoModRef;
    ...
    if (cs1ModExpanded.intersects(cs2RefExpanded) ||
