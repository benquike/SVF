...
/*!
 * This class serves as a base may-happen in parallel analysis for multithreaded
 * program Given a statement under an abstract thread, it tells which abstract
 * threads may be alive at the same time (May-happen-in-parallel).
 */
class MHP {
  ...
  public:
    ...
    inline TCT *getTCT() const { return tct; }

    /// Whether the function is connected from main function in thread call
    /// graph
    bool isConnectedfromMain(const Function *fun);

    //    /// Interface to query whether two instructions are protected by
    //    common locks virtual bool isProtectedByACommonLock(const Instruction*
    //    i1, const Instruction* i2); virtual bool isAllCxtInSameLockSpan(const
    //    Instruction *I1, const Instruction *I2); virtual bool
    //    isOneCxtInSameLockSpan(const Instruction *I1, const Instruction *I2);
    //
    ...
    virtual bool mayHappenInParallelCache(const Instruction *i1,
                                          const Instruction *i2);
    virtual bool mayHappenInParallelInst(const Instruction *i1,
                                         const Instruction *i2);
    virtual bool executedByTheSameThread(const Instruction *i1,
                                         const Instruction *i2);
    ...
    inline bool hasInterleavingThreads(const CxtThreadStmt &cts) const {
        return threadStmtToTheadInterLeav.find(cts) !=
               threadStmtToTheadInterLeav.end();
    }
    //@}

    /// Get/has ThreadStmt
    //@{
    inline const CxtThreadStmtSet &
    getThreadStmtSet(const Instruction *inst) const {
  ...
  private:
    ...
    inline void addInterleavingThread(const CxtThreadStmt &tgr,
                                      const CxtThreadStmt &src) {
        bool changed = threadStmtToTheadInterLeav[tgr] |=
            threadStmtToTheadInterLeav[src];
        if (changed) {
            instToTSMap[tgr.getStmt()].insert(tgr);
            pushToCTSWorkList(tgr);
        }
    }
    inline void rmInterleavingThread(const CxtThreadStmt &tgr,
                                     const NodeBS &tids,
                                     const Instruction *joinsite) {
        NodeBS joinedTids;
        for (NodeBS::iterator it = tids.begin(), eit = tids.end(); it != eit;
             ++it) {
            if (isMustJoin(tgr.getTid(), joinsite))
                joinedTids.set(*it);
        }
        if (threadStmtToTheadInterLeav[tgr].intersectWithComplement(
                joinedTids)) {
            pushToCTSWorkList(tgr);
        }
    }
    ...
    inline void pushCxt(CallStrCxt &cxt, const Instruction *call,
                        const Function *callee) {
        tct->pushCxt(cxt, call, callee);
    }
    /// Match context
    inline bool matchCxt(CallStrCxt &cxt, const Instruction *call,
                         const Function *callee) {
    ...
    /// Return thread id(s) which are directly or indirectly joined at this join
    /// site
    NodeBS getDirAndIndJoinedTid(const CallStrCxt &cxt,
                                 const Instruction *call);

    /// Whether a context-sensitive join satisfies symmetric loop pattern
    const Loop *isJoinInSymmetricLoop(const CallStrCxt &cxt,
    ...
    ThreadStmtToThreadInterleav
        threadStmtToTheadInterLeav; /// Map a statement to its thread
                                    /// interleavings
    InstToThreadStmtSetMap
        instToTSMap; ///< Map an instruction to its ThreadStmtSet
    FuncPairToBool nonCandidateFuncMHPRelMap;

  public:
    u32_t numOfTotalQueries; ///< Total number of queries
    u32_t numOfMHPQueries;   ///< Number of queries are answered as
...
class ForkJoinAnalysis {
  ...
  public:
    ...
    ForkJoinAnalysis(TCT *t) : tct(t) { collectSCEVInfo(); }
    /// functions
    void collectSCEVInfo();

    /// context-sensitive forward traversal from each fork site. Generate
    /// following results (1) fork join pair, maps a context-sensitive join site
    /// to its corresponding thread ids (2) never happen-in-parallel thread
    /// pairs
    ...
    /// Get directly and indirectly joined threadIDs based on a
    /// context-sensitive join site
    ...
    inline bool isFullJoin(NodeID tid1, NodeID tid2) {
        ...
        bool partial =
            partialJoin.find(std::make_pair(tid1, tid2)) != partialJoin.end();
        if (full && !partial)
            return true;
        return false;
    }

    /// Get exit instruction of the start routine function of tid's parent
    /// thread
    inline const Instruction *getExitInstOfParentRoutineFun(NodeID tid) const {
        ...
        const Function *parentRoutine =
            tct->getStartRoutineOfCxtThread(parentct);
  ...
  private:
    ...
    bool sameLoopTripCount(const Instruction *forkSite,
                           const Instruction *joinSite);

    /// Whether it is a matched fork join pair
    bool isAliasedForkJoin(const Instruction *forkSite,
                           const Instruction *joinSite) {
        return tct->getPTA()->alias(getForkedThread(forkSite),
                                    getJoinedThread(joinSite)) &&
    ...
    inline ValDomain getMarkedFlag(const CxtStmt &cs) {
        CxtStmtToAliveFlagMap::const_iterator it =
            cxtStmtToAliveFlagMap.find(cs);
    ...
    void markCxtStmtFlag(const CxtStmt &tgr, const CxtStmt &src) {
        ...
        if (flag_tgr == Empty) {
            ...
            /// alive is at the bottom of the semilattice, nothing needs to be
            /// done here
        }
        if (flag_tgr != getMarkedFlag(tgr)) {
            pushToCTSWorkList(tgr);
    ...
    /// ThreadCallGraph
    inline ThreadCallGraph *getTCG() const { return tct->getThreadCallGraph(); }
    ...
    CxtStmtToAliveFlagMap
        cxtStmtToAliveFlagMap;     ///< flags for context-sensitive statements
    CxtStmtWorkList cxtStmtList;   ///< context-sensitive statement worklist
    CxtStmtToTIDMap directJoinMap; ///< maps a context-sensitive join site to
                                   ///< directly joined thread ids
    CxtStmtToTIDMap
        dirAndIndJoinMap; ///< maps a context-sensitive join site to directly
                          ///< and indirectly joined thread ids
    CxtStmtToLoopMap
        cxtJoinInLoop;      ///< a set of context-sensitive join inside loop
