...
class LockAnalysis {
  ...
  public:
    ...
    LockAnalysis(TCT *t)
        : tct(t), lockTime(0), numOfTotalQueries(0), numOfLockedQueries(0),
          lockQueriesTime(0) {}

    /// context-sensitive forward traversal from each lock site. Generate
    /// following results (1) context-sensitive lock site, (2) maps a
    /// context-sensitive lock site to its corresponding lock span.
    void analyze();
    void analyzeIntraProcedualLock();
    bool intraForwardTraverse(const Instruction *lock, InstSet &unlockset,
                              InstSet &forwardInsts);
    bool intraBackwardTraverse(const InstSet &unlockset,
    ...
    inline void addIntraLock(const Instruction *lockSite,
                             const InstSet &stmts) {
        for (InstSet::const_iterator it = stmts.begin(), eit = stmts.end();
             it != eit; ++it) {
    ...
    /// Add intra-procedural lock
    inline void addCondIntraLock(const Instruction *lockSite,
                                 const InstSet &stmts) {
        for (InstSet::const_iterator it = stmts.begin(), eit = stmts.end();
             it != eit; ++it) {
    ...
    inline bool isInsideIntraLock(const Instruction *stmt) const {
        return instCILocksMap.find(stmt) != instCILocksMap.end() ||
               isInsideCondIntraLock(stmt);
    }

    /// Return true if a statement is inside a partial lock/unlock pair
    ...
    inline void addCxtLock(const CallStrCxt &cxt, const Instruction *inst) {
        ...
        DBOUT(DMTA, SVFUtil::outs() << "LockAnalysis Process new lock ";
              cxtlock.dump());
    }
    ...
    inline bool intersects(const CxtLockSet &lockset1,
                           const CxtLockSet &lockset2) const {
        for (CxtLockSet::const_iterator it = lockset1.begin(),
                                        eit = lockset1.end();
             it != eit; ++it) {
            const CxtLock &lock = *it;
            for (CxtLockSet::const_iterator lit = lockset2.begin(),
                                            elit = lockset2.end();
                 lit != elit; ++lit) {
                if (lock == *lit)
                    return true;
    ...
    inline bool removeCxtStmtToSpan(CxtStmt &cts, const CxtLock &cl) {
        bool find =
            cxtStmtToCxtLockSet[cts].find(cl) != cxtStmtToCxtLockSet[cts].end();
        if (find) {
    ...
    inline void touchCxtStmt(CxtStmt &cts) { cxtStmtToCxtLockSet[cts]; }
    ...
    inline bool hasOneCxtInLockSpan(const Instruction *I,
                                    LockSpan lspan) const {
        if (!hasCxtStmtfromInst(I))
            return false;
        const LockSpan ctsset = getCxtStmtfromInst(I);
        for (LockSpan::const_iterator cts = ctsset.begin(), ects = ctsset.end();
             cts != ects; cts++) {
    ...
    inline bool hasAllCxtInLockSpan(const Instruction *I,
                                    LockSpan lspan) const {
        if (!hasCxtStmtfromInst(I))
            return false;
        const LockSpan ctsset = getCxtStmtfromInst(I);
    ...
    /// we check whether every cxt stmt of instructions is protected by a common
    /// lock.
    bool isProtectedByCommonLock(const Instruction *i1, const Instruction *i2);
    bool isProtectedByCommonCxtLock(const Instruction *i1,
                                    const Instruction *i2);
    bool isProtectedByCommonCxtLock(const CxtStmt &cxtStmt1,
                                    const CxtStmt &cxtStmt2);
    bool isProtectedByCommonCILock(const Instruction *i1,
                                   const Instruction *i2);
    ...
    inline u32_t getNumOfCxtLocks() { return cxtLockset.size(); }
  ...
  private:
    ...
    void handleCallRelation(CxtLockProc &clp, const PTACallGraphEdge *cgEdge,
                            CallSite call);
    ...
    void markCxtStmtFlag(const CxtStmt &tgr, const CxtStmt &src) {
        ...
        if (hasCxtLockfromCxtStmt(tgr) == false) {
            for (CxtLockSet::const_iterator it = srclockset.begin(),
                                            eit = srclockset.end();
                 it != eit; ++it) {
    ...
    bool intersect(CxtLockSet &tgrlockset, const CxtLockSet &srclockset) {
        ...
        for (CxtLockSet::const_iterator it = tgrlockset.begin(),
                                        eit = tgrlockset.end();
             it != eit; ++it) {
            if (srclockset.find(*it) == srclockset.end())
                toBeDeleted.insert(*it);
        }
        for (CxtLockSet::const_iterator it = toBeDeleted.begin(),
                                        eit = toBeDeleted.end();
             it != eit; ++it) {
            tgrlockset.erase(*it);
        }
    ...
    /// Clear flags
    inline void clearFlagMap() { cxtStmtList.clear(); }
    //@}
    ...
    /// Following data structures are used for collecting context-sensitive
    /// locks
    //@{
