...
class MemSSAStat : public PTAStat {
  ...
  public:
    static const char
        *TotalTimeOfConstructMemSSA; ///< Total time for constructing memory SSA
    static const char
        *TimeOfGeneratingMemRegions; ///< Time for allocating regions
    static const char
    ...
    static const char
        *NumOfMaxRegion; ///< Number of max points-to set in region.
    static const char *
        NumOfAveragePtsInRegion; ///< Number of average points-to set in region.
    ...
    static const char
        *NumOfFunHasEntryChi; ///< Number of functions which have entry chi
    static const char
        *NumOfFunHasRetMu; ///< Number of functions which have return mu
...
class SVFGStat : public PTAStat {
  public:
    ...
    void dirVFEdgeStart() { connectDirSVFGEdgeTimeStart = CLOCK_IN_MS(); }

    void dirVFEdgeEnd() { connectDirSVFGEdgeTimeEnd = CLOCK_IN_MS(); }

    void indVFEdgeStart() { connectIndSVFGEdgeTimeStart = CLOCK_IN_MS(); }

    void indVFEdgeEnd() { connectIndSVFGEdgeTimeEnd = CLOCK_IN_MS(); }

    void TLVFNodeStart() { addTopLevelNodeTimeStart = CLOCK_IN_MS(); }

    void TLVFNodeEnd() { addTopLevelNodeTimeEnd = CLOCK_IN_MS(); }

    void ATVFNodeStart() { addAddrTakenNodeTimeStart = CLOCK_IN_MS(); }

    void ATVFNodeEnd() { addAddrTakenNodeTimeEnd = CLOCK_IN_MS(); }

    void sfvgOptStart() { svfgOptTimeStart = CLOCK_IN_MS(); }

    void sfvgOptEnd() { svfgOptTimeEnd = CLOCK_IN_MS(); }
  ...
  private:
    ...
    void calculateNodeDegrees(SVFGNode *node, NodeSet &nodeHasIndInEdge,
