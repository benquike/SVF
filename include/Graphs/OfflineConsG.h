 ...
 * In OCG, a 'ref' node is used to represent the point-to set of a constraint
 * node. 'Nor' means a constraint node of its corresponding ref node.
 */
class OfflineConsG : public ConstraintGraph {
  ...
  protected:
    ...
    NodeToRepMap
        norToRepMap; // for each *a construct a --> rep, i.e., mapping a node of
                     // to a rep node for online constraint solving

  public:
    OfflineConsG(PAG *p)
        : ConstraintGraph(p), nodeToRefMap({}), norToRepMap({}) {
        buildOfflineCG();
    }
    ...
    inline NodeID getOCGRep(NodeID node) const { return getNorRep(node); }
    // Get the OCG node to rep map (this map is const and should not be
    // modified)
    inline const NodeToRepMap &getOCGRepMap() const { return norToRepMap; }
...
template <>
struct GraphTraits<SVF::OfflineConsG *>
    : public GraphTraits<
