...
/**
 * Optimised SVFG.
 * 1. FormalParam/ActualRet is converted into Phi. ActualParam/FormalRet becomes
 * the operands of Phi nodes created at callee/caller's entry/callsite.
 * 2. ActualIns/ActualOuts resides at direct call sites id removed. Sources of
 * its incoming edges are connected with the destinations of its outgoing edges
 * directly.
 * 3. FormalIns/FormalOuts reside at the entry/exit of non-address-taken
 * functions is removed as ActualIn/ActualOuts.
 * 4. MSSAPHI nodes are removed if it have no self cycle. Otherwise depends on
 * user option.
 */
class SVFGOPT : public SVFG {
  ...
  public:
    ...
    inline void setTokeepActualOutFormalIn() { keepActualOutFormalIn = true; }
    inline void setTokeepAllSelfCycle() { keepAllSelfCycle = true; }
    inline void setTokeepContextSelfCycle() { keepContextSelfCycle = true; }
  ...
  protected:
    ...
    //@{
    inline void connectAParamAndFParam(const PAGNode *cs_arg,
    ...
    inline void connectAInAndFIn(const ActualINSVFGNode *actualIn,
    ...
    inline void connectFOutAndAOut(const FormalOUTSVFGNode *formalOut,
    ...
    inline NodeID getFormalOUTDef(NodeID fo) const {
        ...
        assert(it != formalOutToDefMap.end() &&
               "can not find formal-out's def");
        return it->second;
    }
    //@}
  ...
  private:
    ...
    SVFGEdge *addCallIndirectSVFGEdge(NodeID srcId, NodeID dstId,
                                      CallSiteID csid, const PointsTo &cpts);
    /// Add indirect ret edge from src to dst with one call site ID.
    SVFGEdge *addRetIndirectSVFGEdge(NodeID srcId, NodeID dstId,
                                     CallSiteID csid, const PointsTo &cpts);
    //@}

    /// 1. Convert FormalParmSVFGNode into PHISVFGNode and add all
    /// ActualParmSVFGNoe which may propagate pts to it as phi's operands.
    /// 2. Do the same thing for ActualRetSVFGNode and FormalRetSVFGNode.
    /// 3. Record def site of ActualINSVFGNode. Remove all its edges and connect
    /// its predecessors
    ///    and successors.
    /// 4. Do the same thing for FormalOUTSVFGNode as 3.
    /// 5. Remove
    /// ActualINSVFGNode/FormalINSVFGNode/ActualOUTSVFGNode/FormalOUTSVFGNode if
    /// they
    ...
    //@{
    /// Record def sites of actual-in/formal-out and connect from those
    /// def-sites to formal-in/actual-out directly if they exist.
    void retargetEdgesOfAInFOut(SVFGNode *node);
    /// Connect actual-out/formal-in's predecessors to their successors
    /// directly.
    ...
    /// Remove self cycle edges if needed. Return TRUE if some self cycle edges
    /// remained.
    bool checkSelfCycleEdges(const MSSAPHISVFGNode *node);

    /// Add new SVFG edge from src to dst.
    bool addNewSVFGEdge(NodeID srcId, NodeID dstId, const SVFGEdge *preEdge,
    ...
    inline void setActualINDef(NodeID ai, NodeID def) {
        auto it = actualInToDefMap.find(ai);
        assert(it == actualInToDefMap.end() &&
               "can not set actual-in's def twice");
    ...
    inline void setFormalOUTDef(NodeID fo, NodeID def) {
        auto it = formalOutToDefMap.find(fo);
        assert(it == formalOutToDefMap.end() &&
               "can not set formal-out's def twice");
    ...
    inline bool formalInOfAddressTakenFunc(const FormalINSVFGNode *fi) const {
        return (
            fi->getEntryChi()->getFunction()->getLLVMFun()->hasAddressTaken());
    }
    ...
    /// Return TRUE if this node has both incoming call/ret and outgoing
    /// call/ret edges.
    ...
    /// 3. FormalIN if it doesn't reside at the entry of address-taken function
    /// and it's not
    ...
    NodeIDToNodeIDMap
        formalOutToDefMap; ///< map formal-out to its def-site node
