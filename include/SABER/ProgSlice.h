...
class ProgSlice {
  ...
  public:
    ...
    using SVFGNodeToCondMap =
        Map<const SVFGNode *,
            Condition *>; ///< map a SVFGNode to its condition during value-flow
                          ///< guard computation

    using VFWorkList =
        FIFOWorkList<const SVFGNode *>; ///< worklist for value-flow guard
                                        ///< computation
    using CFWorkList =
    ...
    /// Destructor
    virtual ~ProgSlice() { destroy(); }

    inline u32_t getForwardSliceSize() const { return forwardslice.size(); }
    inline u32_t getBackwardSliceSize() const { return backwardslice.size(); }
    ...
    //@{
    inline const SVFGNode *getSource() const { return root; }
    ...
    inline void setPartialReachable() { partialReachable = true; }
    inline void setAllReachable() { fullReachable = true; }
    inline bool setReachGlobal() { return reachGlob = true; }
    inline bool isPartialReachable() const {
        return partialReachable || reachGlob;
    }
    inline bool isAllReachable() const { return fullReachable || reachGlob; }
    inline bool isReachGlobal() const { return reachGlob; }
    //@}
  ...
  protected:
    ...
    //@{
    inline Condition *ComputeIntraVFGGuard(const BasicBlock *src,
                                           const BasicBlock *dst) {
        return pathAllocator->ComputeIntraVFGGuard(src, dst);
    }
    inline Condition *ComputeInterCallVFGGuard(const BasicBlock *src,
                                               const BasicBlock *dst,
                                               const BasicBlock *callBB) {
        return pathAllocator->ComputeInterCallVFGGuard(src, dst, callBB);
    }
    inline Condition *ComputeInterRetVFGGuard(const BasicBlock *src,
                                              const BasicBlock *dst,
    ...
    //@{
    inline const SVFGNode *getCurSVFGNode() const { return _curSVFGNode; }
    ...
    inline void setFinalCond(Condition *cond) { finalCond = cond; }
