...
class PAGBuilder : public llvm::InstVisitor<PAGBuilder> {
  ...
  public:
    /// Constructor
    PAGBuilder(PAG *pag, SVFProject *proj)
        : pag(pag), svfMod(proj->getSVFModule()), proj(proj), curBB(nullptr),
          curVal(nullptr) {}
    /// Destructor
    virtual ~PAGBuilder() {}
    ...
    /// GetObject - Return the object node (stack/global/heap/function)
    /// according to a LLVM Value
    inline NodeID getObjectNode(const Value *V) {
        return pag->getObjectNode(V);
    }

    /// getReturnNode - Return the node representing the unique return value of
    /// a function.
    inline NodeID getReturnNode(const SVFFunction *func) {
        return pag->getReturnNode(func);
    }

    /// getVarargNode - Return the node representing the unique variadic
    ...
    const Type *getBaseTypeAndFlattenedFields(Value *v,
    ...
    void visitCallBrInst(CallBrInst &I) { return visitCallSite(&I); }
    ...
    /// Set current basic block in order to keep track of control flow
    /// information
    inline void setCurrentLocation(const Value *val, const BasicBlock *bb) {
        curBB = bb;
        curVal = val;
    }
    inline const Value *getCurrentValue() const { return curVal; }
    inline const BasicBlock *getCurrentBB() const { return curBB; }

    /// Add global black hole Address edge
    void addGlobalBlackHoleAddrEdge(NodeID node,
    ...
    NodeID getGepValNode(const Value *val, const LocationSet &ls,
    ...
    /// Add Call edge
    inline CallPE *addCallEdge(NodeID src, NodeID dst,
    ...
    /// Add Gep edge
    inline GepPE *addGepEdge(NodeID src, NodeID dst, const LocationSet &ls,
                             bool constGep) {
    ...
    /// Add Offset(Gep) edge
    inline NormalGepPE *addNormalGepEdge(NodeID src, NodeID dst,
    ...
    inline TDForkPE *addThreadForkEdge(NodeID src, NodeID dst,
                                       const CallBlockNode *cs) {
    ...
    inline TDJoinPE *addThreadJoinEdge(NodeID src, NodeID dst,
                                       const CallBlockNode *cs) {
