...
class DCHNode : public GenericNode<DCHNode, DCHEdge> {
  public:
    ...
    /// Flags
    //@{
    inline void setFlag(CLASSATTR mask) { flags |= mask; }
    inline bool hasFlag(CLASSATTR mask) const { return (flags & mask) == mask; }
    //@}

    /// Attribute
    //@{
    inline void setPureAbstract() { setFlag(PURE_ABSTRACT); }
    inline void setMultiInheritance() { setFlag(MULTI_INHERITANCE); }
    inline void setTemplate() { setFlag(TEMPLATE); }
    inline void setScalar() { setFlag(SCALAR); }
    inline bool isPureAbstract() const { return hasFlag(PURE_ABSTRACT); }
    inline bool isMultiInheritance() const {
        return hasFlag(MULTI_INHERITANCE);
    }
    inline bool isTemplate() const { return hasFlag(TEMPLATE); }
    inline bool isScalar() const { return hasFlag(SCALAR); }
    //@}
  ...
  private:
    ...
    /// If a vtable is split into more than one vfn vector for multiple
    /// inheritance, 0 would be the primary base + this classes virtual
    /// functions, 1 would be the second parent, 2 would be third parent, etc.
...
class DCHGraph : public CommonCHGraph, public GenericGraph<DCHNode, DCHEdge> {
  ...
  public:
    ...
    const DIType *getFieldType(const DIType *base, unsigned idx) {
        ...
        // For TBHC this is conservative because the union type is lower in the
        // DCHG than its fields. TODO: make more precise.
        ...
        assert(fieldTypes.find(base) != fieldTypes.end() &&
               "DCHG: base not flattened!");
    ...
    const Set<const DIType *> &getAggs(const DIType *base) {
        base = getCanonicalType(base);
        assert(containingAggs.find(base) != containingAggs.end() &&
               "DCHG: aggregates not gathered for base!");
  ...
  protected:
    ...
    /// Maps aggregate types to all the aggregate types it transitively
    /// contains.
  ...
  private:
    ...
    /// Returns a set of all children of type (CHA). Also gradually builds
    /// chaMap.
 ...
 */
template <>
struct GraphTraits<SVF::DCHNode *>
    : public GraphTraits<SVF::GenericNode<SVF::DCHNode, SVF::DCHEdge> *> {};

/// Inverse GraphTraits specializations for call graph node, it is used for
/// inverse traversal.
template <>
struct GraphTraits<Inverse<SVF::DCHNode *>>
    : public GraphTraits<
          Inverse<SVF::GenericNode<SVF::DCHNode, SVF::DCHEdge> *>> {};

template <>
struct GraphTraits<SVF::DCHGraph *>
    : public GraphTraits<SVF::GenericGraph<SVF::DCHNode, SVF::DCHEdge> *> {
