...
class MemRegion {
  ...
  public:
    ...
    inline MRID getMRID() const { return rid; }
    /// Return points-to
    inline const PointsTo &getPointsTo() const { return cptsSet; }
    /// Operator== overriding
    ...
    inline u32_t getRegionSize() const { return cptsSet.count(); }
};
...
class MRGenerator {
  ...
  public:
    ...
    using PtsToRepPtsSetMap =
        OrderedMap<PointsTo, PointsTo, MemRegion::equalPointsTo>;
  ...
  private:
    ...
    /// Map a load PAG Edge to its memory regions sets in order for inserting
    /// mus in Memory SSA
    LoadsToMRsMap loadsToMRsMap;
    /// Map a store PAG Edge to its memory regions sets in order for inserting
    /// chis in Memory SSA
    ...
    /// Get all the objects in callee's modref escaped via global objects (the
    /// chain pts of globals)
  ...
  protected:
    ...
    /// Given a condition pts, insert into cptsToRepCPtsMap for region
    /// generation
    ...
    virtual inline void getAliasMemRegions(MRSet &aliasMRs,
                                           const PointsTo &cpts,
    ...
    virtual inline void getMRsForCallSiteRef(MRSet &aliasMRs,
                                             const PointsTo &cpts,
                                             const SVFFunction *) {
        const MemRegion *mr = getMR(cpts);
        aliasMRs.insert(mr);
    ...
    virtual void modRefAnalysis(PTACallGraphNode *callGraphNode,
                                WorkList &worklist);

    /// Get Mod-Ref of a callee function
    virtual bool handleCallsiteModRef(NodeBS &mod, NodeBS &ref,
                                      const CallBlockNode *cs,
                                      const SVFFunction *fun);

    /// Add cpts to store/load
    //@{
    inline void addCPtsToStore(PointsTo &cpts, const StorePE *st,
                               const SVFFunction *fun) {
        storesToPointsToMap[st] = cpts;
        funToPointsToMap[fun].insert(cpts);
        addModSideEffectOfFunction(fun, cpts);
    }
    inline void addCPtsToLoad(PointsTo &cpts, const LoadPE *ld,
                              const SVFFunction *fun) {
    ...
    inline FunToPointsToMap &getFunToPointsToList() { return funToPointsToMap; }
    //@}
    ...
    /// Add indirect uses an memory object in the function
    bool addRefSideEffectOfCallSite(const CallBlockNode *cs,
                                    const NodeBS &refs);
    /// Add indirect def an memory object in the function
    bool addModSideEffectOfCallSite(const CallBlockNode *cs,
                                    const NodeBS &mods);
  ...
  public:
    inline Size_t getMRNum() const { return memRegSet.size(); }

    /// Destructor
    virtual ~MRGenerator() { destroy(); }
    ...
    const SVFFunction *getFunction(const PAGEdge *pagEdge) const {
        ...
        assert(it != pagEdgeToFunMap.end() &&
               "can not find its function, it is a global PAG edge");
        return it->second;
    }
    ...
    inline bool hasPAGEdgeList(const Instruction *inst) {
        PAG *pag = pta->getPAG();
        if (ptrOnlyMSSA)
            return pag->hasPTAPAGEdgeList(
                pag->getICFG()->getBlockICFGNode(inst));
    ...
    inline PAGEdgeList &getPAGEdgesFromInst(const Instruction *inst) {
        PAG *pag = pta->getPAG();
        if (ptrOnlyMSSA)
            return pag->getInstPTAPAGEdgeList(
                pag->getICFG()->getBlockICFGNode(inst));
    ...
    /// getModRefInfo APIs
    //@{
    /// Collect mod ref for external callsite other than heap alloc external
    /// call
    ...
    ModRefInfo getModRefInfo(const CallBlockNode *cs1,
