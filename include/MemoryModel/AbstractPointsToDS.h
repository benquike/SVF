...
/// Given a key (variable/condition variable), return its points-to data
/// (pts/condition pts) It is designed flexible for different context, heap and
/// path sensitive analysis Context Insensitive			   Key --> Variable,
/// Data --> PointsTo
/// Context sensitive:  			   Key --> CondVar,  Data --> PointsTo
/// Heap sensitive:     			   Key --> Variable  Data -->
/// CondPointsToSet Context and heap sensitive:     Key --> CondVar,  Data -->
/// CondPointsToSet
///
/// This class is abstract to allow for multiple methods of actually storing
/// points-to sets. Key:   "owning" variable of a points-to set. Datum: elements
/// in points-to sets. Data:  the points-to set; a collection of Datums.
template <typename Key, typename Datum, typename Data> class PTData {
  public:
...
/// The points-to information is propagated incrementally only for the different
/// parts.
...
/// Data-flow points-to data structure for flow-sensitive analysis as defined by
/// Hardekopf and Lin (CGO 11). Points-to information is maintained at each
/// program point (statement). For address-taken variables, every program point
/// has two sets: IN and OUT points-to sets. For top-level variables, points-to
/// sets are maintained flow-insensitively via getPts(var).
template <typename Key, typename Datum, typename Data>
...
/// PTData with normal keys and versioned keys. Replicates the PTData interface
/// for versioned keys too. Intended to be used for versioned flow-sensitive
/// PTA--hence the name--but can be used anywhere where there are two types of
/// keys at play.
...
class VersionedPTData : public PTData<Key, Datum, Data> {
  public:
    ...
    virtual const VersionedKeySet &
    getVersionedKeyRevPts(const Datum &datum) = 0;

    virtual bool addPts(const VersionedKey &vk, const Datum &element) = 0;

    virtual bool unionPts(const VersionedKey &dstVar,
                          const VersionedKey &srcVar) = 0;
    ...
    ///@{
    static inline bool
    classof(const VersionedPTData<Key, Datum, Data, VersionedKey> *) {
        return true;
    }
