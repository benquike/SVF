...
class MutablePTData : public PTData<Key, KeySet, Data, DataSet> {
  ...
  public:
    ...
    inline bool unionPts(const Key &dstKey,
                         const DataSet &srcDataSet) override {
    ...
    virtual inline void dumpPts(const PtsMap &ptsSet,
        ...
        for (PtsMapConstIter nodeIt = ptsSet.begin(); nodeIt != ptsSet.end();
            ...
            if (pts.empty())
                continue;
            O << var << " ==> { ";
            for (typename DataSet::iterator cit = pts.begin(), ecit = pts.end();
                 cit != ecit; ++cit) {
                O << *cit << " ";
            }
            O << "}\n";
  ...
  private:
    ...
    inline bool addPts(DataSet &d, const Data &e) { return d.test_and_set(e); }
    inline void addSingleRevPts(KeySet &revData, const Key &tgr) {
        if (this->rev)
            insertKey<Key, KeySet>(tgr, revData);
    }
...
class MutableDiffPTData : public DiffPTData<Key, KeySet, Data, DataSet> {
  ...
  protected:
    ...
    inline DataSet &getMutDiffPts(Key &var) { return diffPtsMap[var]; }

    /// Get propagated points to.
    inline DataSet &getPropaPts(Key &var) { return propaPtsMap[var]; }
...
class IncMutableDFPTData : public MutableDFPTData<Key, KeySet, Data, DataSet> {
  public:
    ...
    using LocID = typename BaseDFPTData::LocID;
    using UpdatedVarMap = Map<LocID, DataSet>; ///< for propagating only newly
...
/// for VersionedKeys.
template <typename Key, typename KeySet, typename Data, typename DataSet,
          typename VersionedKey, typename VersionedKeySet>
class MutableVersionedPTData
    : public VersionedPTData<Key, KeySet, Data, DataSet, VersionedKey,
                             VersionedKeySet> {
  public:
    using BasePTData = PTData<Key, KeySet, Data, DataSet>;
    using BaseVersionedPTData = VersionedPTData<Key, KeySet, Data, DataSet,
