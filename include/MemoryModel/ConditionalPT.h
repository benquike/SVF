...
class CondVar {
  public:
    ...
    inline bool operator!=(const CondVar &rhs) const { return !(*this == rhs); }
    ///@}
    ...
    inline NodeID get_id() const { return m_id; }
...
class CondStdSet {
  ...
  public:
    ...
    CondStdSet(const CondStdSet<Element> &cptsSet)
        : elements(cptsSet.getElementSet()) {}
    ...
    inline void set(const Element &var) { elements.insert(var); }
    /// Remove var from the set.
    inline void reset(const Element &var) { elements.erase(var); }

    /// Set size
    //@{
    inline bool empty() const { return elements.empty(); }
    inline unsigned size() const { return elements.size(); }
    inline unsigned count() const { return size(); }
    //@}
    ...
    inline iterator begin() const { return elements.begin(); }
    inline iterator end() const { return elements.end(); }
    //@}
    ...
    inline const ElementSet &getElementSet() const { return elements; }
...
class CondPointsToSet {
  public:
    ...
    //@{
    inline CondPts &pointsTo(void) { return _condPts; }
    inline const CondPts &pointsTo(void) const { return _condPts; }
    ...
    inline PointsTo &pointsTo(Cond cond) { return _condPts[cond]; }
    //@}

    /// iterators
    //@{
    inline CondPtsIter cptsBegin() { return _condPts.begin(); }
    inline CondPtsIter cptsEnd() { return _condPts.end(); }
    inline CondPtsConstIter cptsBegin() const { return _condPts.begin(); }
    inline CondPtsConstIter cptsEnd() const { return _condPts.end(); }
    //@}

    inline void clear() { _condPts.clear(); }
    ...
    inline bool empty() const { return numElement() == 0; }

    /// Overloading operators
    //@{
    inline CondPointsToSet<Cond> &
    operator=(const CondPointsToSet<Cond> &other) {
    ...
    inline bool operator&=(const CondPointsToSet<Cond> &rhs) {
        if (empty()) {
            ...
            bool changed = false;
            for (CondPtsIter it = cptsBegin(), eit = cptsEnd(); it != eit;
                 ++it) {
                const Cond &cond = it->first;
                PointsTo &pts = it->second;
     ...
     * points-to elements.
     * 1. CondPointsToSet with smaller points-to set size is smaller than the
     * other;
     * 2. If the sizes are equal, comparing the conditions and real points-to
     * targets in their points-to elements.
     */
    ...
    inline std::string dumpStr() const {
        ...
        for (; it != eit; it++) {
            const PointsTo &pts = it->second;
            str += "pts{";
            for (NodeBS::iterator ii = pts.begin(), ie = pts.end(); ii != ie;
                 ii++) {
                char int2str[16];
      ...
      public:
        CondPtsSetIterator(CondPointsToSet<Cond> &n, bool ae = false)
            : _curIter(n.cptsBegin()), _endIter(n.cptsEnd()),
              ...
              _ptEndIter(_curIter->second.end()), atEnd(ae) {}
        ~CondPtsSetIterator() {}
        bool operator!=(int val) { return _curIter != _endIter; }
        ...
        Cond cond() { return _curIter->first; }
  ...
  public:
    ...
    inline iterator end() const { return iterator(this, true); }
    //@}
