...
template <class CVar, class CPtSet, class DPIm> class DDAVFSolver {
  ...
  public:
    ...
    virtual bool unionDDAPts(DPIm dpm, const CPtSet &targetPts) {
        CPtSet &pts = isTopLevelPtrStmt(dpm.getLoc()) ? dpmToTLCPtSetMap[dpm]
                                                      : dpmToADCPtSetMap[dpm];
        return pts |= targetPts;
    }
    /// Add pts
  ...
  protected:
                    ...
                    if (propagateViaObj(*it, getLoadCVar(dpm))) {
                        ...
                        if (isStrongUpdate(
                            ...
                            backtraceAlongIndirectVF(
                                pts, getDPImWithOldCond(dpm, *it, node));
                        }
                    } else {
    ...
    inline void OOBResetVisited() {
        for (typename LocToDPMVecMap::const_iterator
                 it = locToDpmSetMap.begin(),
    ...
    void backtraceAlongIndirectVF(CPtSet &pts, const DPIm &oldDpm) {
        ...
        for (auto *it : edgeSet) {
            if (const IndirectSVFGEdge *indirEdge =
                ...
                if (guard.test(obj)) {
                    DBOUT(DDDA, SVFUtil::outs()
                                    << "\t\t==backtrace indirectVF svfgNode "
                                    << indirEdge->getDstID() << " --> "
                                    << indirEdge->getSrcID() << "\n");
                    backwardPropDpm(pts, oldDpm.getCurNodeID(), oldDpm,
                                    indirEdge);
                }
            }
        }
    ...
    void backtraceAlongDirectVF(CPtSet &pts, const DPIm &oldDpm) {
        ...
        for (auto *it : edgeSet) {
            if (const DirectSVFGEdge *dirEdge =
                ...
                backwardPropDpm(pts,
                                getSVFG()->getLHSTopLevPtr(srcNode)->getId(),
                                oldDpm, dirEdge);
            }
        }
    }
    ...
    inline void startNewPTCompFromLoadSrc(CPtSet &pts, const DPIm &oldDpm) {
        ...
        DBOUT(DDDA, SVFUtil::outs()
                        ...
                        << "\n");
        const SVFGEdge *edge =
            getSVFG()->getIntraVFGEdge(loadSrc, load, SVFGEdge::IntraDirectVF);
    ...
    inline void startNewPTCompFromStoreDst(CPtSet &pts, const DPIm &oldDpm) {
        ...
        DBOUT(DDDA, SVFUtil::outs()
                        ...
                        << "\n");
        const SVFGEdge *edge = getSVFG()->getIntraVFGEdge(
            storeDst, store, SVFGEdge::IntraDirectVF);
        assert(edge && "Edge not found!!");
    ...
    inline void backtraceToStoreSrc(CPtSet &pts, const DPIm &oldDpm) {
        ...
        DBOUT(DDDA, SVFUtil::outs() << "++backtrace to storeSrc from svfgNode "
                                    ...
                                    << storeSrc->getId() << "\n");
        const SVFGEdge *edge = getSVFG()->getIntraVFGEdge(
            storeSrc, store, SVFGEdge::IntraDirectVF);
        assert(edge && "Edge not found!!");
    ...
    virtual void backwardPropDpm(CPtSet &pts, NodeID ptr, const DPIm &oldDpm,
        ...
        if (handleBKCondition(dpm, edge) == false) {
            DOTIMESTAT(ddaStat->_TotalTimeOfBKCondition +=
                       DDAStat::getClk() - start);
            DBOUT(DDDA, SVFUtil::outs() << "\t!!! infeasible path svfgNode: "
                                        << edge->getDstID() << " --| "
                                        << edge->getSrcID() << "\n");
    ...
    virtual bool isMustAlias(const DPIm &, const DPIm &) { return false; }
    ...
    void resolveFunPtr(const DPIm &dpm) {
        if (const CallBlockNode *cbn =
            ...
            CallInstSet csSet;
            /// use pre-analysis call graph to approximate all potential
            /// callsites
            _ander->getPTACallGraph()->getIndCallSitesInvokingCallee(fun,
                                                                     csSet);
            for (const auto *it : csSet) {
    ...
    /// Update call graph
    virtual inline void updateCallGraphAndSVFG(const DPIm &,
                                               const CallBlockNode *,
                                               SVFGEdgeSet &) {}
    //@}
