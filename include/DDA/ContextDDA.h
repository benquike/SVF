...
class ContextDDA : public CondPTAImpl<ContextCond>,
  ...
  public:
    ...
    /// Finalize analysis
    inline void finalize() override { CondPTAImpl<ContextCond>::finalize(); }
    ...
    /// (1) concrete calling context (not involved in recursion and not exceed
    /// the maximum context limit) (2) not inside loop
    bool isHeapCondMemObj(const CxtVar &var,
                          const StoreSVFGNode *store) override;

    /// refine indirect call edge
    bool testIndCallReachability(CxtLocDPItem &dpm, const SVFFunction *callee,
    ...
    inline virtual bool isEdgeInRecursion(CallSiteID csId) {
        const SVFFunction *caller =
            getPTACallGraph()->getCallerOfCallSite(csId);
        const SVFFunction *callee =
            getPTACallGraph()->getCalleeOfCallSite(csId);
        return inSameCallGraphSCC(caller, callee);
    }
    /// Update call graph.
    //@{
    void updateCallGraphAndSVFG(const CxtLocDPItem &dpm,
                                const CallBlockNode *cs,
                                SVFGEdgeSet &svfgEdges) override {
    ...
    /// processGep node
    CxtPtSet processGepPts(const GepSVFGNode *gep,
                           const CxtPtSet &srcPts) override;
    ...
    /// same
    inline bool propagateViaObj(const CxtVar &storeObj,
                                const CxtVar &loadObj) override {
    ...
    /// (most recent ids to objects): compatible : (e.g., 123 == 123, 123 ==
    /// 23). not compatible (e.g., 123 != 423)
    inline bool isCondCompatible(const ContextCond &cxt1,
