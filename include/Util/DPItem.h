 ...
 * This serves as a base class for CFL-reachability formulation by matching
 * parentheses. Extend this class for further sophisticated CFL-reachability
 * items (e.g. field, flow, path)
 */
class DPItem {
  protected:
    NodeID cur;
  ...
  public:
    ...
    /// Destructor
    virtual ~DPItem() {}
    inline NodeID getCurNodeID() const { return cur; }
    inline void setCurNodeID(NodeID c) { cur = c; }
    /// set max step budge per query
    static inline void setMaxBudget(u32_t max) { maximumBudget = max; }
    static inline u32_t getMaxBudget() { return maximumBudget; }
    ...
    inline bool operator<(const DPItem &rhs) const { return cur < rhs.cur; }
    ...
    inline bool operator==(const DPItem &rhs) const { return (cur == rhs.cur); }
    /// Overloading Operator!=
    inline bool operator!=(const DPItem &rhs) const { return !(*this == rhs); }

    inline void dump() const { SVFUtil::outs() << "var " << cur << "\n"; }
};
...
template <class LocCond> class StmtDPItem : public DPItem {
  ...
  public:
    ...
    /// Set location
    inline void setLoc(const LocCond *l) { this->curloc = l; }
...
class ContextCond {
  public:
    ...
    ContextCond(const ContextCond &cond)
        : context(cond.getContexts()), concreteCxt(cond.isConcreteCxt()) {}
    /// Destructor
    virtual ~ContextCond() {}
    /// Get context
    inline const CallStrCxt &getContexts() const { return context; }
    /// Get context
    inline CallStrCxt &getContexts() { return context; }
    /// Whether it is an concrete context
    inline bool isConcreteCxt() const { return concreteCxt; }
    /// Whether it is an concrete context
    inline void setNonConcreteCxt() { concreteCxt = false; }
    ...
    /// Get context size
    inline u32_t cxtSize() const { return context.size(); }
    /// set max context limit
    static inline void setMaxCxtLen(u32_t max) { maximumCxtLen = max; }
    /// Push context
    ...
    /// Begin iterators
    inline const_iterator begin() const { return context.begin(); }
    /// End iterators
    inline const_iterator end() const { return context.end(); }
...
template <class LocCond> class CxtStmtDPItem : public StmtDPItem<LocCond> {
  ...
  public:
    ...
    /// Get context
    inline const ContextCond &getCond() const { return this->context; }
    /// Get context
    inline ContextCond &getCond() { return this->context; }
    /// Push context
    ...
    operator=(const CxtStmtDPItem<LocCond> &rhs) {
        if (*this != rhs) {
...
class VFPathCond : public ContextCond {
  ...
  public:
    ...
    VFPathCond(const VFPathCond &cond)
        : ContextCond(cond), path(cond.getPaths()), edges(cond.getVFEdges()) {}
    /// Destructor
    virtual ~VFPathCond() {}
    /// set max path limit
    static inline void setMaxPathLen(u32_t max) { maximumPathLen = max; }
    inline u32_t getMaxPathLen() const { return maximumPathLen; }
    /// Return paths
    inline PathCond *getPaths() const { return path; }
    /// Return paths
    inline const EdgeSet &getVFEdges() const { return edges; }
    /// Set paths
    ...
    /// Get path length
    inline u32_t pathLen() const { return edges.size(); }
    /// Add SVFG Edge
    ...
    inline bool addPath(PathCondAllocator *allocator, PathCond *c, NodeID from,
                        NodeID to) {
...
template <class LocCond> class PathStmtDPItem : public StmtDPItem<LocCond> {
  ...
  public:
    ...
    inline const VFPathCond &getCond() const { return this->vfpath; }
    /// Get value-flow paths
    inline VFPathCond &getCond() { return this->vfpath; }
    /// Add a value-flow path (avoid adding duplicated paths)
    inline bool addVFPath(PathCondAllocator *allocator, PathCond *c,
    ...
    /// Match context
    bool matchContext(NodeID cxt) { return this->vfpath.matchContext(cxt); }
    ...
    operator=(const PathStmtDPItem<LocCond> &rhs) {
        if (*this != rhs) {
            StmtDPItem<LocCond>::operator=(rhs);
...
class CxtDPItem : public DPItem {
  ...
  public:
    ...
    /// Push context
    inline void pushContext(NodeID cxt) { context.pushContext(cxt); }
