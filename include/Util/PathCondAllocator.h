...
class PathCondAllocator {
  ...
  public:
    ...
    using CondPosMap =
        Map<u32_t, Condition *>; ///< map a branch to its Condition
    using BBCondMap =
        Map<const BasicBlock *, CondPosMap>; // map bb to a Condition
    using CondToTermInstMap =
        Map<const Condition *,
            const Instruction *>; // map a condition to its branch instruction
    using BasicBlockSet = Set<const BasicBlock *>;
    using FunToExitBBsMap =
        Map<const Function *,
            BasicBlockSet>; ///< map a function to all its basic blocks calling
                            ///< program exit
    using BBToCondMap =
        Map<const BasicBlock *,
            Condition *>; ///< map a basic block to its condition during
                          ///< control-flow guard computation
    using CFWorkList =
        FIFOWorkList<const BasicBlock *>; ///< worklist for control-flow guard
                                          ///< computation
    ...
    inline Condition *condNeg(Condition *cond) { return bddCondMgr->NEG(cond); }
    inline Condition *getTrueCond() const { return bddCondMgr->getTrueCond(); }
    ...
    inline void printDbg(Condition *cond) { bddCondMgr->printDbg(cond); }
    ...
    //@{
    virtual Condition *ComputeIntraVFGGuard(const BasicBlock *src,
                                            const BasicBlock *dst);
    virtual Condition *ComputeInterCallVFGGuard(const BasicBlock *src,
                                                const BasicBlock *dst,
                                                const BasicBlock *callBB);
    virtual Condition *ComputeInterRetVFGGuard(const BasicBlock *src,
                                               const BasicBlock *dst,
                                               const BasicBlock *retBB);

    /// Get complement condition (from B1 to B0) according to a complementBB
    /// (BB2) at a phi e.g., B0: dstBB; B1:incomingBB; B2:complementBB
    virtual Condition *getPHIComplementCond(const BasicBlock *BB1,
                                            const BasicBlock *BB2,
                                            const BasicBlock *BB0);

    inline void clearCFCond() { bbToCondMap.clear(); }
    /// Set current value for branch condition evaluation
    inline void setCurEvalVal(const Value *val) { curEvalVal = val; }
    /// Get current value for branch condition evaluation
    inline const Value *getCurEvalVal() const { return curEvalVal; }
    //@}
  ...
  private:
    ...
    void setBranchCond(const BasicBlock *bb, const BasicBlock *succ,
                       Condition *cond);
    /// Get branch condition
    Condition *getBranchCond(const BasicBlock *bb,
                             const BasicBlock *succ) const;
    /// Get a condition, evaluate the value for conditions if necessary (e.g.,
    ...
    Condition *evaluateLoopExitBranch(const BasicBlock *bb,
                                      const BasicBlock *succ);
    /// Return branch condition after evaluating test null like expression
    Condition *evaluateTestNullLikeExpr(const BranchInst *brInst,
                                        const BasicBlock *succ,
                                        const Value *val);
    /// Return condition when there is a branch calls program exit
    Condition *evaluateProgExit(const BranchInst *brInst,
                                const BasicBlock *succ);
    ...
    bool isTestContainsNullAndTheValue(const CmpInst *cmp,
    ...
    inline Condition *createNewCond(u32_t i) {
        assert(indexToDDNodeMap.find(i) == indexToDDNodeMap.end() &&
    ...
    CondToTermInstMap condToInstMap; ///< map a condition to its corresponding
                                     ///< llvm instruction
    PTACFInfoBuilder cfInfoBuilder;  ///< map a function to its loop info
    FunToExitBBsMap funToExitBBsMap; ///< map a function to all its basic blocks
                                     ///< calling program exit
    BBToCondMap bbToCondMap; ///< map a basic block to its path condition
                             ///< starting from root
    const Value *curEvalVal; ///< current llvm value to evaluate branch
                             ///< condition when computing guards
