...
class WPAMinimumSolver : public WPASCCSolver<GraphType> {
  ...
  protected:
    virtual void solve() {
        bool solveAll = true;
        /// If the worklist is not empty, then only solve these nodes contained
  ...
  private:
    inline void addNewCandidate(NodeID node) { candidates.set(node); }
    inline const NodeBS &getCandidates() const { return candidates; }
    inline void removeCandidates(const NodeBS &nodes) {
        candidates.intersectWithComplement(nodes);
    }

    NodeBS
        candidates; ///< nodes which need to be analyzed in current iteration.
};
