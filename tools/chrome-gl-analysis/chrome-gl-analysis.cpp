...
static llvm::cl::opt<std::string>
    Entry("e", llvm::cl::desc("entry function name"),
...
void traverseFunctionICFG(ICFG *icfg, SVFModule *mod,
  ...
  while (!worklist.empty()) {
    ...
    } else if (const auto *cbnode = SVFUtil::dyn_cast<CallBlockNode>(vNode)) {
      const Instruction *inst = cbnode->getCallSite();
      visitor.visit(const_cast<Instruction*>(inst));
    } else {
      cout << vNode->getId() << " is not an IntraBlockNode, skipping" << endl;
    }
...
class MyBrConditionVisitor: public InstVisitor<MyBrConditionVisitor> {
  ...
  public:
    ...
    MyBrConditionVisitor(set<const Value *> &res):
      res(res) {
    }
...
void analyzeArgFlowToCondition(const SVFG *vfg,
                               const Value *val,
  ...
  for (const auto *node : visited) {
    ...
    if (const auto *parmVFGNode = SVFUtil::dyn_cast<FormalParmVFGNode>(node)) {
